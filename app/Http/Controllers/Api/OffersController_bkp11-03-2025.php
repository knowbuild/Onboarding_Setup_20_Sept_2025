...
$order_ids = array_map(function($item) {
    $offer_data = json_decode($item->offer_data);
	  $tasks_details_array=getTaskList($offer_data->orders_id);
	  $product_items_details=product_name_generated_with_quantity_json_tbl_order_product_listing_new_without_json_stringfy($offer_data->orders_id);
     $offer_data->orders_id;
	
	$array_data= array("orders_id"=>$offer_data->orders_id,
	"company_full_name"=>$offer_data->company_full_name,
	
	"price_type"=>$offer_data->Price_type,	
	"customers_contact_no"=>$offer_data->customers_contact_no,	
	"time_lead_added"=>$offer_data->time_lead_added,
	"days_since_offer"=>$offer_data->days_since_offer,
	"application_name"=>$offer_data->application_name,
	"cust_segment_name"=>$offer_data->cust_segment_name,
	"enq_source_name"=>$offer_data->enq_source_name,
	"orders_status"=>$offer_data->orders_status,
	"ensure_sale_month"=>$offer_data->ensure_sale_month,
	"ref_source"=>$offer_data->ref_source,
	"proforma_invoice_id"=>$offer_data->proforma_invoice_id,
	"proforma_invoice_send_for_approval"=>$offer_data->proforma_invoice_send_for_approval,
	"proforma_invoice_status"=>$offer_data->proforma_invoice_status,
	"admin_name"=>$offer_data->admin_name,
	"offer_probability"=>$offer_data->offer_probability,
	"lead_id"=>$offer_data->lead_id,
	"payment_terms"=>$offer_data->payment_terms,
	"order_in_favor_of"=>$offer_data->order_in_favor_of,
	"order_by"=>$offer_data->order_by,
	"subject"=>$offer_data->subject,
	"show_discount"=>$offer_data->show_discount,
	"delivery_terms"=>$offer_data->delivery_terms,
	"offer_warranty"=>$offer_data->offer_warranty,
	"total_order_cost"=>$offer_data->total_order_cost,
	"total_order_cost_new"=>$offer_data->total_order_cost_new,
	"edited_enq_id"=>$offer_data->edited_enq_id,
	"master_enq_id"=>$offer_data->master_enq_id,
	
	"dead_duck"=>$offer_data->dead_duck,	
		"offer_type"=>$offer_data->offer_type,
"hot_offer"=>$offer_data->hot_offer,
	"time_ordered"=>$offer_data->time_ordered,
	"date_ordered"=>$offer_data->date_ordered,
	"offer_currency"=>$offer_data->offer_currency,
	"billing_company"=>$offer_data->billing_company,
	"shipping_company"=>$offer_data->shipping_company,
	"app_cat_id"=>$offer_data->app_cat_id,
"freight_amount"=>$offer_data->freight_amount,
"offer_validity"=>$offer_data->offer_validity,
"follow_up_date"=>$offer_data->follow_up_date,
"order_in_favor_of"=>$offer_data->order_in_favor_of,
"delivery_terms"=>$offer_data->delivery_terms,

"enq_remark_edited"=>$offer_data->enq_remark_edited,
	"city_name"=>$offer_data->city_name,
	"state_name"=>$offer_data->state_name,
	"country_name"=>$offer_data->country_name,
	"shipping_city"=>$offer_data->shipping_city,
	"shipping_state"=>$offer_data->shipping_state,
		"shipping_zip_code"=>$offer_data->shipping_zip_code,
	"shipping_country"=>$offer_data->shipping_country,
		"shipping_address"=>$offer_data->shipping_address,
		
		"customers_email"=>$offer_data->customers_email,
	"customers_name"=>$offer_data->customers_name,
		"comp_person_id"=>$offer_data->comp_person_id,
		"customers_id"=>$offer_data->customers_id,

...
public function getPurchaseOrders(Request $request)
{
  ...
  $lastPurchases = DB::table('vendor_po_order')
    ...
    $grouped = $results->groupBy('PO_ID')->map(function ($items) use ($partialPoData, $vendorMasters, $vendorContacts, $poTotals, $poApprovalStatuses, $lastPurchases) {
        $item = $items->first();
...
public function purchasealerts(Request $request)
{
    ...
    $enrichedResults = $results->map(function ($row) use ($fyStart, $fyEnd, $lastFyStart, $lastFyEnd, $currentFyLabel, $lastFyLabel) {
        ...
        // Calculate price change for YTD
        ...
        // Calculate price change for Last FY
        if ($ytdPrice == 0 && $lastFyPrice > 0) {
            ...
            $priceChangeLastFY = ($priceValueLastFY > 0) ? "+{$priceValueLastFY}% 	&uarr;" : (($priceValueLastFY < 0) ? "{$priceValueLastFY}%" : '0%');
        }
...
// Calculate % and absolute change for YTD
if ($lastFyPrice == 0 && $ytdPrice > 0) {
    $priceChangeYTD = '+100%';
    $priceValueYTD = 100;
} elseif ($lastFyPrice == 0 && $ytdPrice == 0) {
...
// Calculate % and absolute change for Last FY
if ($ytdPrice == 0 && $lastFyPrice > 0) {
    $priceChangeLastFY = '-100%';
    $priceValueLastFY = -100;
} elseif ($ytdPrice == 0 && $lastFyPrice == 0) {
...
public function stockbreach(Request $request)
{
    ...
    $finalResults = $results->map(function ($row) use ($startDate, $endDate, $fyStart, $fyEnd, $lastFyStart, $lastFyEnd, $currentFyLabel, $lastFyLabel) {
        ...
        $poHistory = getPoHistoryFinalByProductId($pro_id);
        $row->po_history = array_values($poHistory instanceof \Illuminate\Support\Collection ? $poHistory->toArray() : ($poHistory ?? []));
...
public function topVendorsByFY(Request $request)
{
    try {
        ...
        $previousFyEnd = (clone $currentFyEnd)->subYear();

        $currentFyLabel = $currentFyStart->format('Y') . '-' . $currentFyEnd->format('y');
        $previousFyLabel = $previousFyStart->format('Y') . '-' . $previousFyEnd->format('y');

        $fetchVendorStats = function ($start, $end) {
            return DB::table('vendor_po_final as pf')
                ...
                ->groupBy(function ($item) {
                    return $item->vendor_id . '|' . $item->currency;
                });
        };

        $currentFyData = $fetchVendorStats($currentFyStart, $currentFyEnd);
        $previousFyData = $fetchVendorStats($previousFyStart, $previousFyEnd);

        $merged = [];

        foreach ($currentFyData as $key => $currentList) {
            $current = $currentList->first();
            $previous = $previousFyData->get($key)?->first();

            $merged[] = [
                'vendor_name' => $current->vendor_name,
                'currency' => $current->currency,
                'current_fy_po_count' => (int) $current->po_count,
                'current_fy_value' => (float) $current->total_value,
                'previous_fy_po_count' => $previous ? (int) $previous->po_count : 0,
                'previous_fy_value' => $previous ? (float) $previous->total_value : 0,
            ];
        }

        foreach ($previousFyData as $key => $previousList) {
            if (!$currentFyData->has($key)) {
                $previous = $previousList->first();
                $merged[] = [
                    'vendor_name' => $previous->vendor_name,
                    'currency' => $previous->currency,
                    'current_fy_po_count' => 0,
                    'current_fy_value' => 0,
                    'previous_fy_po_count' => (int) $previous->po_count,
                    'previous_fy_value' => (float) $previous->total_value,
                ];
            }
        }

        // Sort by current_fy_value descending
        usort($merged, fn($a, $b) => $b['current_fy_value'] <=> $a['current_fy_value']);

        // Handle pagination
        $page = (int) $request->input('pageno', 1);
        $perPage = (int) $request->input('records', 50);
        $total = count($merged);
        $offset = ($page - 1) * $perPage;
        $pagedData = array_slice($merged, $offset, $perPage);
...
public function createDraftPurchaseOrder(Request $request)
{
    ...
    try {
        ...
        // 1. Get product by vendor + pro_id
        ...
        // 3. Get buyer from default branch
        $buyer = DB::table('tbl_company_branch_address')->where('default_branch', 1)->first();
        if (!$buyer) {
        ...
        // 7. Format legacy text blocks
//        $buyerFormatted = "{$buyer->company_name}\n{$buyer->address},\n{$buyer->city} - {$buyer->pincode}\nTel: {$buyer->telephone}\nEmail: {$buyer->email}\nGST: {$buyer->gst_no}";
	//	$buyerFormatted = "{$buyer->company_name}\n{$buyer->address},\n{$buyer->city} - {$buyer->pincode}\nTel: " . ($buyer->telephone ?? '') . "\nEmail: " . ($buyer->email ?? '') . "\nGST: " . ($buyer->gst_no ?? '');
    ...
    'GST: ' . ($buyer->gst_no ?? '')
        ...
        DB::table('vendor_po_final')->insert([
            ...
            // Consignee (Vendor�s manufacturing address)

...
public function storeVendorProduct(Request $request)
{
    $validator = app('validator')->make($request->all(), [
        ...
        'internalItemCode'    => 'required|string|max:150',
        'pro_id_entry'        => 'nullable|integer',
        'pro_id'              => 'nullable|integer',
        'vendorItemCode'      => 'required|string|max:150',
...
$currencyDetails = currencySymbolByCurrencyCode($order->currency);
        $currency_symbol = $currencyDetails[0] ?? null;
        $currency_value = $currencyDetails[1] ?? null;
        $currency_css_symbol = $currencyDetails[2] ?? null;
        return [
            'ID' => $order->O_ID,
...
public function getVendorProductsforPo(Request $request)
{
    ...
    // Safe UPC Sync: update only null/0 upc_code in vendor_product_list
    DB::table('vendor_product_list')
        ->join('tbl_products', 'vendor_product_list.pro_id', '=', 'tbl_products.pro_id')
        ->whereNotNull('tbl_products.upc_code')
        ->where('tbl_products.upc_code', '!=', 0)
        ->where(function ($q) {
            $q->whereNull('vendor_product_list.upc_code')
              ->orWhere('vendor_product_list.upc_code', 0);
        })
        ->update([
            'vendor_product_list.upc_code' => DB::raw('tbl_products.upc_code')
        ]);

    $query = DB::table('vendor_product_list')
        ->join('vendor_master', 'vendor_product_list.Vendor_List', '=', 'vendor_master.ID')
        ->leftJoin('tbl_products', 'vendor_product_list.pro_id', '=', 'tbl_products.pro_id')
        ->select(
            'vendor_product_list.ID as vendor_product_listing_id',
            'vendor_product_list.ACL_Item_Code as internalItemCode',
            'vendor_product_list.Vendor_Item_Code as vendorItemCode',
            ...
            'vendor_product_list.pro_id',
            'vendor_product_list.Vendor_List as vendor_id',
            'vendor_product_list.upc_code as vendor_upc_code',
            'tbl_products.upc_code as master_upc_code',
            DB::raw("CASE 
                        WHEN tbl_products.upc_code IS NOT NULL AND tbl_products.upc_code != 0 
                            THEN tbl_products.upc_code 
                        WHEN vendor_product_list.upc_code IS NOT NULL AND vendor_product_list.upc_code != 0 
                            THEN vendor_product_list.upc_code 
                        ELSE NULL 
                     END as upc_code"),
            'vendor_product_list.category as category',
            'vendor_master.C_name as Vendor_name',
        ...
        $query->where(function ($q) use ($searchKeyword, $searchType) {
            switch ($searchType) {
                case 'vendor_name':
                    ...
                    break;
                case 'vendor_id':
                    $q->where('vendor_master.ID', $searchKeyword);
                    break;
                case 'product_name':
...
public function editPurchaseOrder(Request $request)
{
    try {
        ...
        DB::table('vendor_po_order')->where('ID', $poId)->update([
			...
			'freightValue' => $data['requirementDetails']['freightValue'] ?? 0,
        ...
        foreach ($requirements as $item) {
            ...
            DB::table('vendor_po_final')->insert([
				...
				'Freight_Value' => $data['requirementDetails']['freightValue'] ?? 0,
...
if (!$vendor) {
 ...
 //  $priceBasisRaw = $vendor->Currency ?? 'INR';
   
   $currency=$vendor->Currency;
    $currencyDetails = currencySymbolByCurrencyCode($currency);
        $currency_symbol = $currencyDetails[0] ?? null;
        $currency_value = $currencyDetails[1] ?? null;
        $currency_css_symbol = $currencyDetails[2] ?? null;
 ...
 // Determine financial year if not provided
    ...
    $poTotals = DB::table('vendor_po_final')
        ->selectRaw('
            ...
            SUM(Prodcut_Qty * Prodcut_Price) + SUM((Prodcut_Qty * Prodcut_Price) * (Tax_Value / 100)) AS grand_total
        ')
        ->groupBy('PO_ID')
        ->get()
        ->keyBy('PO_ID');
    ...
    $rawData = DB::table('vendor_po_order')
        ->leftJoin('vendor_master', 'vendor_master.ID', '=', 'vendor_po_order.VPI')
...
*/
 ...
 if ($fromDate && $toDate) {
    ...
    $rawData->whereBetween('vendor_po_order.received_date', [$startDate, $endDate]);
}
    $results = $rawData->get();
    ...
    $allItems = collect($poMap)->map(function ($items) use ($partialPoData, $draftPoItems, $poTotals) {
        $item = $items[0];
        ...
        if ($item->u_pro_ID) {
            ...
            $item->po_total = $tot->grand_total;
        } else {
            $draft = $draftPoItems[$item->PO_ID] ?? (object)['total' => 0];
            $item->po_total = $draft->total;
        }
    ...
    $summary = [
        ...
        'Open'      => ['count' => 0, 'total_value' => 0],
        'Partial'   => ['count' => 0, 'total_value' => 0],
    ...
    foreach ($allItems as $i) {
        ...
        if (!isset($summary[$status])) {
            Log::warning("Unexpected PO Status: $status for PO_ID " . $i->PO_ID);
            continue;
        }

        $summary[$status]['count']++;
        $summary[$status]['total_value'] += $i->po_total ?? 0;
    }

    return response()->json([
        'summary' => collect($summary)->map(fn ($v, $k) => [
            'status' => $k,
            'count' => $v['count'],
            'total_value' => number_format((float) $v['total_value'], 2, '.', '')
        ])->values(),

        //  Root-level totals
        'total_draft'     => $summary['Draft']['count'],
        'total_open'      => $summary['Open']['count'],
        'total_partial'   => $summary['Partial']['count'],
...
public function getPurchaseOrderSummary_old_19_july_2025(Request $request) 
{
    $today = Carbon::now();
...
public function createpofinal(Request $request)
{
    ...
    try {
		...
		$flag 		= $data['currency'] ?? '' ;
		//$flag 		= strtolower($data['poDetails']['type'] ?? '') === 'domestic' ? 'INR' : 'USD';
...
$requirements = $poFinalRows->map(function ($item, $index) {
    ...
    // Fallback logic if UPC or HSN is blank or zero
    $upc = $item->upc_code;
    $hsn = $item->hsn_code;

    if (empty($upc) || $upc == '0') {
        $upc = pro_upc_code($pro_id); // Replace with actual helper
    }

    if (empty($hsn) || $hsn == '0') {
        $hsn = ProHsn_code($pro_id); // Replace with actual helper
    }
...
/*
    ...
    $taxPercent = $poFinal->Tax_Value ?? 0;
	$freightValue = $poFinal->Freight_Value ?? 0;

    $subTotal1 = $requirements->sum('subTotal');
 	$subTotal = $subTotal1 + $freightValue;
    $taxValue = ($taxPercent / 100) * $subTotal;
$purchase_type = $vendorMaster->purchase_type ?? null;
    // Determine IGST or CGST+SGST
    $vendorStateId = $poOrder->exporter_state ?? ($vendorMaster->State ?? null);
    $buyerStateId = $poOrder->buyer_state ?? ($buyerMaster->state ?? null);

    $isIntraState = ($vendorStateId && $buyerStateId && $vendorStateId == $buyerStateId);

    if ($isIntraState) {
        $calculatedTaxType = 'CGST+SGST';
        $cgst = round($taxValue / 2, 2);
        $sgst = round($taxValue / 2, 2);
        $igst = 0;
    } else {
        $calculatedTaxType = 'IGST';
        $cgst = 0;
        $sgst = 0;
        $igst = round($taxValue, 2);
    }

    $poTotal = $subTotal + $taxValue;
	$amt_in_words=amountInWords($poTotal);
...
//$currency_symbol=currencySymbolByCurrencyCode($poFinal->Flag);


        $currencyDetails = currencySymbolByCurrencyCode($poFinal->Flag);
        $currency_symbol = $currencyDetails[0] ?? null;
        $currency_value = $currencyDetails[1] ?? null;
        $currency_css_symbol = $currencyDetails[2] ?? null;
    ...
    return response()->json([
        ...
        'consigneeDetails' => [
           ...
           // 'contactName' => $getValue('consignee_contact_name', 'consignee_contact_name') ?: (($buyerMaster->company_name ?? '') . ' delhi'),
			//'contactName' => $getValue('consignee_contact_name', 'consignee_contact_name')   ?: (isset($buyerMaster->company_name) ? ($buyerMaster->company_name . ' delhi') : ''),
			'contactName' => $getValue('consignee_contact_name', 'consignee_contact_name') ?: '',
        ...
        'termsDetails' => [
            ...
            'paymentTermsName' => vendor_payment_terms_name($poFinal->Payment_Terms) ?? '',
            'priceBasis' => $priceBasisId,
            'priceBasis_name' => $priceBasisName,
            'warranty' => $poFinal->Warranty ?? '',
            'warranty_name' => warranty_name($poFinal->Warranty) ?? '',
        ...
        'requirementDetails' => [
            ...
            'calculated_tax_type' => $calculatedTaxType,
...
try {
    if (!empty($item->Date) && $item->Date !== '0000-00-00') {
        $item->days_since = Carbon::parse($item->Date)->diffInDays(Carbon::now());
    } else {
...
public function purchasealerts(Request $request)
{
    ...
    $currencyType = $request->input('currency_type', 'default_currency');
    $page = max(1, (int) $request->input('pageno', 1));
    $limit = max(1, (int) $request->input('records', 10));
    $offset = ($page - 1) * $limit;

    // Map filter param (snake_case)- labels
    $filterPoTypeParam = strtolower($request->input('po_type_filter', 'none'));
    $filterMap = [
        'draft_po_created' => 'Draft PO Created',
        'final'            => 'Final',
        'po_not_created'   => 'PO NOT Created'
    ];
    $filterPoType = $filterMap[$filterPoTypeParam] ?? 'none';
    ...
    $baseQuery = DB::table('tbl_delivery_order')
        ...
        ->whereNull('tbl_delivery_challan.Invoice_No')
        ->whereBetween('tbl_delivery_order.D_Order_Date', [$fyStart->format('Y-m-d'), $fyEnd->format('Y-m-d')])
        ->where('tbl_delivery_order.D_Order_Date', '!=', '0000-00-00')
        ->where('tbl_delivery_order.do_type', '!=', 'service')
        ->groupBy('tbl_do_products.pro_id', 'tp.pro_id', 'tp.ware_house_stock', 'tp.reorder_qty')
        ->havingRaw('SUM(tbl_do_products.Quantity) > tp.ware_house_stock');
    ...
    $fullData = $baseQuery
        ->select(
            ...
            'tpe.model_no',
            'tpe.pro_desc_entry as product_description',
            DB::raw('GROUP_CONCAT(DISTINCT tbl_delivery_order.O_Id) as related_orders'),
            'tpe.hsn_code',
            'tp.upc_code',
            DB::raw('MAX(tbl_delivery_order.D_Order_Date) as latest_order_date'),
            'tp.ware_house_stock',
            'tp.reorder_qty',
            'ta.application_name',
            DB::raw('SUM(tbl_do_products.Quantity) as recommended_quantity')
        )
        ->get();
    ...
    $totalRecords = DB::table(DB::raw("({$baseQuery->toSql()}) as sub"))
        ->mergeBindings($baseQuery)
        ->count();
    $paginatedData = $fullData->slice($offset, $limit)->values();
    $proIds = $paginatedData->pluck('pro_id')->toArray();

    $invoiceQtyMap = getTotalQtyInvoiceBulk($fyStart->toDateString(), $fyEnd->toDateString(), $proIds);
    $poSummaryMap = getPoSummariesBulk($proIds, $fyStart, $fyEnd, $lastFyStart, $lastFyEnd, $currencyType);
    $poHistoryMap = getPoHistoriesBulk($proIds, $currencyType);

    $currencyInfo = getDefaultCurrencyInfo();
    $defaultCurrencySymbol = $currencyInfo['currency_symbol'];
    $defaultCurrencyCode   = $currencyInfo['currency_code'];

    $result = [];

    foreach ($paginatedData as $row) {
        $pro_id = $row->pro_id;
        $invoiceQty = $invoiceQtyMap[$pro_id] ?? 0;
        $poHistories = $poHistoryMap[$pro_id] ?? [];

        //  Determine PO label (Final / Draft / None)
        $hasDraft = false;
        $hasFinal = false;
        foreach ($poHistories as $po) {
            if (isset($po['po_type'])) {
                if ($po['po_type'] === 0) $hasFinal = true;
                if ($po['po_type'] === 1) $hasDraft = true;
            }
        }
        ...
        $daysSinceBreach = Carbon::parse($row->latest_order_date)->diffInDays(Carbon::now());
        ...
        $result[] = [
            ...
            'alert_date' => $row->latest_order_date,
            'days_since_stock_breach' => $daysSinceBreach,
            'last_purchase_price' => $poHistories[0]['rate'] ?? 0,
            'category_name' => $row->application_name,
            'last_purchase_quantity' => $poHistories[0]['quantity'] ?? 0,
            'current_stock' => $row->ware_house_stock,
            'recommended_quantity' => $row->recommended_quantity,
            'minimum_reorder_stock_level' => $row->reorder_qty,
            'invoiced_quantity_to_be_fulfilled' => $invoiceQty,
            'currency' => $defaultCurrencyCode,
            'currency_symbol' => $defaultCurrencySymbol,
            'po_created_label' => $poCreatedLabel,
            'po_history' => $poHistories
        ];
    }

    // Apply filter if requested (snake_case param mapped to label)
    if ($filterPoType !== 'none') {
        $result = array_values(array_filter($result, fn($r) => $r['po_created_label'] === $filterPoType));
    }

    return response()->json([
        'status' => true,
        'purchase_alerts' => $result,
...
public function purchasealerts_old8_8_2025(Request $request)
{
    $currentDate = Carbon::now();
    $fyStart = Carbon::create($currentDate->month >= 4 ? $currentDate->year : $currentDate->year - 1, 4, 1);
    $fyEnd = (clone $fyStart)->addYear()->subSecond();
   ...
   /* $defaultCurrencyInfo = getDefaultCurrency();
    $defaultCurrencyCode = $defaultCurrencyInfo['code'] ?? '';
    $defaultCurrencySymbol = $defaultCurrencyInfo['symbol'] ?? '';*/
	
	 $currencyInfo = getDefaultCurrencyInfo();
    $defaultCurrencySymbol = $currencyInfo['currency_symbol'];
    $defaultCurrencyCode   = $currencyInfo['currency_code'];
    $defaultCurrencyValue  = $currencyInfo['currency_value'];
    ...
    foreach ($paginatedData as $row) {
        ...
        $ytd = round($poSummaryMap[$pro_id]['ytd']['value'] ?? 0, 2);
        $lastFy = round($poSummaryMap[$pro_id]['last_fy']['value'] ?? 0, 2);
        $invoiceQty = $invoiceQtyMap[$pro_id] ?? 0;
        $poHistories = $poHistoryMap[$pro_id] ?? [];

        // Include both Draft and Final POs
        $hasPo = !empty($poHistories);
        $poCreatedLabel = $hasPo ? 'PO Created' : 'PO Not Created';
...
public function purchaseforecasting(Request $request)
{
    ...
    $filterPoType = $request->input('po_type_filter');
    $currencyType = $request->input('currency_type'); // default_currency or import_currency
    ...
    // Currency defaults
    $currencies = DB::table('tbl_currencies')->pluck('currency_value', 'currency_code');
    $defaultCurrency = DB::table('tbl_currencies')->where('currency_status', 'yes')->first();
    $defaultCode = $defaultCurrency->currency_code;
    $defaultSymbol = $defaultCurrency->currency_symbol;
    $defaultRate = $defaultCurrency->currency_value;

    // Step 1: Fetch main product sales funnel
    $results = DB::table('tbl_order as t1')
        ->join('tbl_order_product as t2', 't1.orders_id', '=', 't2.order_id')
        ->join('tbl_products as tp', 't2.pro_id', '=', 'tp.pro_id')
        ...
        ->select(
            ...
            't1.offer_probability',
            DB::raw('SUM(t2.pro_quantity) as total_product_qty_ordered'),
            DB::raw('MAX(t1.date_ordered) as latest_order_date'),
			DB::raw('GROUP_CONCAT(DISTINCT t1.orders_id) as related_orders')
        )
        ->when($startDate && $endDate, fn($query) => $query->whereBetween('t1.date_ordered', [$startDate, $endDate]))
        ->where('tp.ware_house_stock', '>', 0)
        ->groupBy('tp.pro_id')
        ->orderByDesc('t1.date_ordered')
        ->get();

    $productIds = $results->pluck('pro_id')->toArray();

    

// ---- Attach PO info ----
foreach ($results as &$product) {
    $poGroup = $poTypes[$product->pro_id] ?? collect();

    if ($poGroup->isEmpty()) {
        $product->po_created_label = 'PO NOT Created';
        $product->po_id = null;
        $product->received_date = null;
    } else {
        // Flexible detection (numeric or string)
        $hasFinal = $poGroup->contains(fn($po) => $po->po_type == 0 || strtolower($po->po_type) === 'final');
        $hasDraft = $poGroup->contains(fn($po) => $po->po_type == 1 || strtolower($po->po_type) === 'draft');
        ...
        if ($hasFinal) {
            ...
            $status = 'PO NOT Created';
        }

        $firstPo = $poGroup->first();
        $product->po_created_label = $status;
        $product->po_id = $firstPo->PO_ID ?? null;
        $product->received_date = $firstPo->received_date ?? null;
    }
}

// ---- Apply filter AFTER PO mapping ----
$filterMap = [
    'draft_po_created' => 'Draft PO Created',
    'final'            => 'Final',
    'po_not_created'   => 'PO NOT Created',
...
if (array_key_exists($filterPoType, $filterMap)) {
    $results = $results->filter(fn($p) => 
        isset($p->po_created_label) && $p->po_created_label === $filterMap[$filterPoType]
    )->values();
}
    // Helper data
    ...
    $finalResults = $results->map(function ($row) use (
        $currentFyLabel, $lastFyLabel, $invoiceQtyMap, $poSummaryMap, $poHistoryMap
    ) {
        $pro_id = $row->pro_id;

        // Days since latest order
        try {
            if (!empty($row->latest_order_date) && $row->latest_order_date !== '0000-00-00') {
                $row->days_since_stock_breach = Carbon::parse($row->latest_order_date)->diffInDays(Carbon::now());
            } else {
                $row->days_since_stock_breach = null;
            }
        } catch (\Exception $e) {
            $row->days_since_stock_breach = null;
        }
        ...
        // Recommended qty
        $row->recommended_quantity = ceil(($row->ware_house_stock + $row->reorder_qty) * ($conversionRatePer / 100));

        // Alt formula
        $row->recommended_quantity_alt = (($salesFunnel * $conversionRate) - ($row->current_stock ?? 0)) + ($row->minimum_reorder_stock_level ?? 0);
        $row->recommended_quantity_alt = $row->recommended_quantity_alt < 0 ? 0 : round($row->recommended_quantity_alt);
    ...
    // Filter: remove rows where recommended_quantity < 1
    $finalResults = $finalResults->filter(fn($row) => $row->recommended_quantity >= 1)->values();

    // Total before pagination
    $totalRecords = $finalResults->count();

    // Paginate after filtering
    $pagedResults = $finalResults->slice($offset, $records)->values();
...
public function purchaseforecasting_old_13_aug_2025(Request $request)
{
    ...
    $startDate = $request->input('start_date', $fyStart->toDateString());
    $endDate = $request->input('end_date', $fyEnd->toDateString());
	$filterPoType = $request->input('po_type_filter'); // values: 'draft', 'final', 'all'
/*    $perPage = (int) $request->input('records', 10);
    $page = (int) $request->input('pageno', 1);
    $offset = ($page - 1) * $perPage;
...
/*    ->select(
 ...
 $totalCount = "100";
   ...
   $poTypes = DB::table('vendor_po_final as pf')
    ->join('vendor_po_order as po', 'pf.PO_ID', '=', 'po.ID')
    ->where('po.Confirm_Purchase', 'inactive')
    ->whereIn('pf.pro_id', $productIds)
    ->select('pf.pro_id', 'po.po_type', 'po.ID as po_id', 'po.received_date')
    ->get()
    ->groupBy('pro_id');

$poTypeStatusMap = [];	

foreach ($poTypes as $proId => $poTypeGroup) {
    $finalPo = $poTypeGroup->firstWhere('po_type', 0);

    if ($finalPo) {
        $poTypeStatusMap[$proId] = [
            'status' => 'final',
            'po_id' => $finalPo->po_id,
            'received_date' => $finalPo->received_date,
        ];
    } else {
        $draftPo = $poTypeGroup->first(); // any draft
        $poTypeStatusMap[$proId] = [
            'status' => 'draft',
            'po_id' => $draftPo->po_id ?? null,
            'received_date' => $draftPo->received_date ?? null,
        ];
    }
}
// For products without PO info
foreach ($results as &$product) {
    if (!isset($product->po_created_label)) {
        $product->po_created_label = 'none';
        $product->po_id = null;
    }
}


// Apply filter if po_type_filter is set
if (in_array($filterPoType, ['draft','final','none'])) {
    $results = $results->filter(fn($p) => $p->po_created_label === $filterPoType)->values();
}

$poTypes = DB::table('vendor_po_final as pf')
    ->join('vendor_po_order as po', 'pf.PO_ID', '=', 'po.ID')
	->where('po.Confirm_Purchase', '!=', 'active')
    ->whereIn('pf.pro_id', $productIds)
    ->select('pf.pro_id', 'pf.PO_ID', 'po.po_type')
    ->get()
    ->groupBy('pro_id');
...
foreach ($poTypes as $proId => $poTypeGroup) {
    $hasFinal = $poTypeGroup->contains(function ($row) {
        return $row->po_type == 0;
    });

    $status = $hasFinal ? 'final' : 'draft';

    // Get first PO_ID (or customize to get latest)
    $poId = optional($poTypeGroup->first())->PO_ID ?? null;
    ...
    // Attach to matching product in results
    foreach ($results as &$product) {
        if (($product->pro_id ?? null) == $proId) {
            $product->po_created_label = $status;
            $product->po_id = $poId;
        }
    }
...
$poTypeMap = getPoTypeByProIdBulk($productIds);
    $invoiceQtyMap = getTotalQtyInvoiceBulk($startDate, $endDate, $productIds);
    $poSummaryMap = getPoSummariesBulk($productIds, $fyStart, $fyEnd, $lastFyStart, $lastFyEnd);
    $poHistoryMap = getPoHistoriesBulk($productIds);
    ...
    // Invoiced quantity for this product
    $invoicedQty = $invoiceQtyMap[$pro_id] ?? 0;
    ...
    //  Calculate recommended_quantity_sql in PHP (no dependency on SQL)
    $row->recommended_quantity = ceil(($row->ware_house_stock + $row->reorder_qty) * ($conversionRatePer / 100));

    // Alt recommended quantity formula
    $row->recommended_quantity_alt = (($salesFunnel * $conversionRate) - ($row->current_stock ?? 0)) + ($row->minimum_reorder_stock_level ?? 0);
    $row->recommended_quantity_alt = $row->recommended_quantity_alt < 0 ? 0 : round($row->recommended_quantity_alt);
    ...
    // Days since stock breach
    if (!empty($row->D_Order_Date) && $row->D_Order_Date !== '0000-00-00') {
        try {
            $orderDate = Carbon::parse($row->D_Order_Date);
            $row->days_since_stock_breach = $orderDate->diffInDays(Carbon::now());
        } catch (\Exception $e) {
            $row->days_since_stock_breach = null;
        }
    } else {
...
public function purchaseforecasting_bkp_live_12_aug_2025(Request $request)
{
    $now = Carbon::now();
    ...
    $forecastData = $rawData->map(function ($row) use (
		...
		if (!empty($row->date_ordered) && $row->date_ordered !== '0000-00-00') {
    try {
        $orderedDate = Carbon::parse($row->date_ordered);
        $row->days_since_stock_breach = $orderedDate->diffInDays(Carbon::now());
    } catch (\Exception $e) {
        $row->days_since_stock_breach = null;
    }
} else {
...
public function stockbreach(Request $request)
{
    ...
    $startDate = $request->input('start_date', $fyStart->toDateString());
    $endDate = $request->input('end_date', $fyEnd->toDateString());
    $filterPoType = $request->input('po_type_filter', 'all'); // values: 'Draft PO Created', 'Final', 'PO NOT Created', 'all'
    $perPage = (int) $request->input('records', 10);
    $page = (int) $request->input('pageno', 1);
    $offset = ($page - 1) * $perPage;
    ...
    $baseQuery = DB::table('tbl_delivery_order')
        ...
        ->leftJoin('tbl_products_entry as tpe', 'tp.pro_id', '=', 'tpe.pro_id')
        ->leftJoin('tbl_application as ta', 'tpe.app_cat_id', '=', 'ta.application_id')
        ->whereNull('tbl_delivery_challan.id')
        ->whereBetween(DB::raw("STR_TO_DATE(tbl_delivery_order.D_Order_Date, '%Y-%m-%d')"), [$startDate, $endDate])
        ->where('tbl_delivery_order.D_Order_Date', '!=', '0000-00-00')
        ->groupBy('t2.pro_id')
        ->havingRaw('SUM(t2.Quantity) > tp.ware_house_stock AND tp.ware_house_stock < tp.reorder_qty');

    $totalCount = (clone $baseQuery)->get()->count();

    $results = $baseQuery
        ->select([
            'tp.pro_id',
            DB::raw('(SUM(t2.Quantity) - tp.ware_house_stock) AS recommended_quantity'),
            'tp.admin_moq as recommended_quantity_mo',
            'tp.pro_title',
            'tp.ware_house_stock as current_stock',
            'tp.reorder_qty as minimum_reorder_stock_level',
            'tpe.app_cat_id as pro_category_id',
            ...
            'tpe.pro_desc_entry as description',
            DB::raw('GROUP_CONCAT(DISTINCT tbl_delivery_order.O_Id) as related_orders'),
            DB::raw('MAX(tbl_delivery_order.D_Order_Date) as D_Order_Date'),
            't2.pro_id as DO_product_pro_id',
            DB::raw('MAX(tbl_delivery_order.D_Order_Date1) as D_Order_Date1'),
            DB::raw('SUM(t2.Quantity) as total_product_qty_ordered'),
            'tbl_delivery_challan.id as delivery_challan_id',
            'ta.application_name',
            'tpe.app_cat_id',
        ])
        ->orderBy(DB::raw('MAX(tbl_delivery_order.D_Order_Date)'), 'desc')
        ->offset($offset)
        ->limit($perPage)
        ->get();
    ...
    $poTypes = DB::table('vendor_po_final as pf')
        ...
        ->where('po.Confirm_Purchase', '!=', 'active')
        ->whereIn('pf.pro_id', $productIds)
        ->select('pf.pro_id', 'pf.PO_ID', 'po.po_type', 'po.received_date')
        ->get()
        ->groupBy('pro_id');
...
foreach ($results as &$product) {
    ...
    if ($poGroup->isEmpty()) {
        ...
        $hasFinal = $poGroup->contains(fn($po) => $po->po_type == 0);
        $hasDraft = $poGroup->contains(fn($po) => $po->po_type == 1);
...
if (array_key_exists($filterPoType, $filterMap)) {
    $results = $results->filter(fn($p) => $p->po_created_label === $filterMap[$filterPoType])->values();
}
    $totalCount = $results->count();

    // ---- Your existing summary mappings ----
    $poTypeMap = getPoTypeByProIdBulk($productIds);
    $invoiceQtyMap = getTotalQtyInvoiceBulk($startDate, $endDate, $productIds);
    $poSummaryMap = getPoSummariesBulk($productIds, $fyStart, $fyEnd, $lastFyStart, $lastFyEnd);
    $poHistoryMap = getPoHistoriesBulk($productIds);
    ...
    $finalResults = $results->map(function ($row) use (
        ...
        // ---- keep your existing mapping logic untouched ----
        $pro_id = $row->pro_id;
        $invoicedQty = $invoiceQtyMap[$pro_id] ?? 0;
        $salesFunnel = (float) $row->total_product_qty_ordered;
        $conversionRate = $salesFunnel > 0 ? ($invoicedQty / $salesFunnel) : 0;
        ...
        $row->recommended_quantity_sql = $row->recommended_quantity;
        $row->recommended_quantity_alt = (($salesFunnel * $conversionRate) - $row->current_stock) + $row->minimum_reorder_stock_level;
        $row->recommended_quantity_alt = $row->recommended_quantity_alt < 0 ? 0 : round($row->recommended_quantity_alt);
        $row->invoiced_total_quantity_of_product = (int) $invoicedQty;
        $row->conversion_rate = round($conversionRatePer, 2) . '%';
        $row->sales_funnel = $salesFunnel;

        // ... (YTD & last FY calc unchanged)
        $summary = $poSummaryMap[$pro_id] ?? [];
        $ytd = $summary['ytd'] ?? [];
        $lastFy = $summary['last_fy'] ?? [];
...
public function stockbreach_old_before_filter_(Request $request)
{
    $today = Carbon::now();
    $fyStart = Carbon::create($today->month >= 4 ? $today->year : $today->year - 1, 4, 1);
    $fyEnd = (clone $fyStart)->addYear()->subDay();
...
/*		->whereNotIn('tp.pro_id', function ($subQuery) {
    $subQuery->select('vendor_po_item.pro_id')
        ->from('vendor_po_order')
        ->join('vendor_po_item', 'vendor_po_item.O_ID', '=', 'vendor_po_order.ID')
        ->where('vendor_po_order.po_type', '=', 1);
})*/
...
$results = $baseQuery
    ->select([
        'tp.pro_id',
      //  DB::raw('GREATEST(IFNULL(SUM(t2.Quantity), 0) - tp.ware_house_stock, 0) as recommended_quantity'),
        ...
        'tpe.pro_desc_entry as description',
        DB::raw('GROUP_CONCAT(DISTINCT tbl_delivery_order.O_Id) as related_orders'),
        DB::raw('MAX(tbl_delivery_order.D_Order_Date) as D_Order_Date'),  // <- FIXED HERE
        't2.pro_id as DO_product_pro_id',
		DB::raw('MAX(tbl_delivery_order.D_Order_Date1) as D_Order_Date1'), // Added key
    ...
    ->orderBy(DB::raw('MAX(tbl_delivery_order.D_Order_Date)'), 'desc')  // Optional: for correct ordering
    ->offset($offset)
    ->limit($perPage)
    ->get();
...
$poTypeMap = getPoTypeByProIdBulk($productIds);
    ...
    $finalResults = $results->map(function ($row) use (
        ...
        $pro_id = $row->pro_id;
//$row->po_types = $poTypeMap[$pro_id] ?? [];

//$row->po_types = $poTypeMap[(string)$row->pro_id] ?? [];
        $invoicedQty = $invoiceQtyMap[$pro_id] ?? 0;
        $salesFunnel = (float) $row->total_product_qty_ordered;
        $conversionRate = $salesFunnel > 0 ? ($invoicedQty / $salesFunnel) : 0;
        $conversionRatePer = $conversionRate * 100;
//        $recommendedQty = (($salesFunnel * $conversionRate) - $row->current_stock) + $row->minimum_reorder_stock_level;
      //  $recommendedQty = $recommendedQty < 0 ? 0 : round($recommendedQty);
$row->recommended_quantity_sql = $row->recommended_quantity; // from SQL
$row->recommended_quantity_alt = (($salesFunnel * $conversionRate) - $row->current_stock) + $row->minimum_reorder_stock_level;
$row->recommended_quantity_alt = $row->recommended_quantity_alt < 0 ? 0 : round($row->recommended_quantity_alt);
...
public function stockbreach_old_30_july_2025(Request $request)
{
    $today = Carbon::now();
    $fyStart = Carbon::create($today->month >= 4 ? $today->year : $today->year - 1, 4, 1);
    $fyEnd = (clone $fyStart)->addYear()->subDay();
...
public function topVendorsByFY(Request $request)
{
    try {
         ...
         * ------------------------------------------------------------ */
        $financialYear = trim($request->input('financial_year'));
        $currFyParam   = trim($request->query('curr_fy_year'));
        $prevFyParam   = trim($request->query('prev_fy_year'));

        // Helper: extract starting year from formats like "2025", "2025-2026"
        ...
        if ($financialYear) {
            $currentFyStart  = $toFyStart($financialYear);
            $previousFyStart = $currentFyStart ? (clone $currentFyStart)->subYear() : null;
        } else {
            $currentFyStart = $toFyStart($currFyParam)
                ?? Carbon::create($today->month >= 4 ? $today->year : $today->year - 1, 4, 1);

            $previousFyStart = $toFyStart($prevFyParam)
                ?? (clone $currentFyStart)->subYear();
        }
        ...
        /* ------------------------------------------------------------
         * 2.  Fetch vendor stats for each FY
         * ------------------------------------------------------------ */
...
//YOY
/*public function purchaseTrendsYOY(Request $request)
{
    // 1. REQUEST PARAMS & CORE DATES
    $financialYear  = trim($request->input('financial_year'));
    $supRef         = $request->input('sup_ref');
    $flag           = trim($request->input('flag')) ?: null;
    $productSearch  = trim($request->input('product_search'));
    $currencyType   = $request->input('currency_type'); // NEW: domestic_currency or import_currency
    ...
    // Get currency symbol for current context if available
    $currencyFlag = DB::table('vendor_po_final')
        ->whereBetween('Date', [$currFYStart, $currFYEnd])
        ->when($supRef, fn($q) => $q->where('Sup_Ref', $supRef))
        ->when(!is_null($flag), fn($q) => $q->where('Flag', $flag))
        ->value('Flag');

    $currencySymbol = currencySymbolByCurrencyCode($currencyFlag)[0] ?? '';

    // NEW: Fetch currency rates (for domestic_currency INR conversion)
    $currencyRates = DB::table('tbl_currencies')
        ->where('currency_status', 'yes')
        ->where('deleteflag', 'active')
        ->pluck('currency_value', 'currency_code')
        ->toArray();
    ...
    for ($fy = $currFYStart->year - 4; $fy <= $currFYStart->year; $fy++) {
        ...
        // NEW: Currency conversion if domestic_currency
        $yearlyRaw = (clone $makeQuery($from, $to))
            ->select('Flag', DB::raw('SUM(Prodcut_Qty * Prodcut_Price) as total'))
            ->groupBy('Flag')
            ->get();

        $total = 0;
        foreach ($yearlyRaw as $entry) {
            $multiplier = 1;
            if ($currencyType === 'domestic_currency') {
                $multiplier = $currencyRates[$entry->Flag] ?? 1;
            }
    ...
    $productsBaseQuery = DB::table('vendor_po_final as vpf')
        ->leftJoin('tbl_products as tp', 'vpf.pro_id', '=', 'tp.pro_id')
        ->whereBetween('vpf.Date', [$prevFYStart, $currFYEnd])
        ->when($supRef, fn($q) => $q->where('vpf.Sup_Ref', $supRef))
        ->when(!is_null($flag), fn($q) => $q->where('vpf.Flag', $flag))
        ->when($productSearch, fn($q) => $q->where('tp.pro_title', 'like', "%{$productSearch}%"))
        ->select(
            ...
            'vpf.Model_No',
            'tp.pro_title as product_name',
            DB::raw("SUM(CASE WHEN vpf.Date BETWEEN '{$currFYStart}' AND '{$currFYEnd}' THEN vpf.Prodcut_Qty ELSE 0 END) AS curr_qty"),
            DB::raw("ROUND(AVG(CASE WHEN vpf.Date BETWEEN '{$currFYStart}' AND '{$currFYEnd}' THEN vpf.Prodcut_Price END), 2) AS curr_price_unit"),
            DB::raw("SUM(CASE WHEN vpf.Date BETWEEN '{$currFYStart}' AND '{$currFYEnd}' THEN vpf.Prodcut_Qty * vpf.Prodcut_Price END) AS curr_total"),
            DB::raw("SUM(CASE WHEN vpf.Date BETWEEN '{$prevFYStart}' AND '{$prevFYEnd}' THEN vpf.Prodcut_Qty ELSE 0 END) AS prev_qty"),
            DB::raw("ROUND(AVG(CASE WHEN vpf.Date BETWEEN '{$prevFYStart}' AND '{$prevFYEnd}' THEN vpf.Prodcut_Price END), 2) AS prev_price_unit"),
            DB::raw("SUM(CASE WHEN vpf.Date BETWEEN '{$prevFYStart}' AND '{$prevFYEnd}' THEN vpf.Prodcut_Qty * vpf.Prodcut_Price END) AS prev_total")
        )
        ->groupBy('vpf.pro_id');
    ...
    // 6. CURRENCY-WISE TOTALS
    $currencyRaw = DB::table('vendor_po_final as vpf')
        ->whereBetween('Date', [$currFYStart, $currFYEnd])
        ->when($supRef, fn($q) => $q->where('Sup_Ref', $supRef))
        ->select('vpf.Flag as currency_code', DB::raw('SUM(vpf.Prodcut_Qty * vpf.Prodcut_Price) as amount'))
        ->whereNotNull('Flag')
        ->groupBy('Flag')
        ->get();

    $currencyWiseTotals = [];
    foreach ($currencyRaw as $row) {
        $symbolData = currencySymbolByCurrencyCode($row->currency_code);
        $amount = round($row->amount, 2);

        if ($currencyType === 'domestic_currency') {
            $conversionRate = $currencyRates[$row->currency_code] ?? 1;
            $amount = round($amount * $conversionRate, 2);
        }

        $currencyWiseTotals[] = [
            'currency_code'   => $row->currency_code,
            'currency_symbol' => $symbolData[0] ?? '',
            'amount'          => $amount,
        ];
    }
    ...
    return response()->json([
        ...
        'year_on_year'        => $yoY,
        'currencyWiseTotals'  => $currencyWiseTotals,
        'pagination' => [
            'pageno'        => $pageno,
            'records'       => $records,
            'total_records' => $totalTopProducts,
...
public function purchaseTrendsYOY(Request $request)
{
    ...
    $fy = explode('-', $financialYear);
    $fromYear = (int) $fy[0];
    $toYear   = (int) $fy[1];
    $fromDate = "$fromYear-04-01";
    $toDate   = "$toYear-03-31";

    $currencyList        = DB::table('tbl_currencies')->where('deleteflag', 'active')->pluck('currency_value', 'currency_code')->toArray();
    $currencySymbols     = DB::table('tbl_currencies')->where('deleteflag', 'active')->pluck('currency_symbol', 'currency_code')->toArray();
    $currencySymbolsHtml = DB::table('tbl_currencies')->where('deleteflag', 'active')->pluck('currency_html_code', 'currency_code')->toArray();
    $domesticCode        = DB::table('tbl_currencies')->where('currency_super_default', 'yes')->value('currency_code') ?? 'INR';

    $currencyRates = $currencyList;

    // ------------------ Product Base Query ------------------
    $productBaseQuery = DB::table('vendor_po_final as vpf')
        ->join('vendor_po_order as vpo', 'vpf.PO_ID', '=', 'vpo.ID')
        ->select(
            DB::raw("DATE_FORMAT(vpf.Date, '%Y-%m-%d') as date"),
            'vpf.PO_ID as po_id',
            'vpf.pro_id as pro_id',
            'vpf.Model_No as Model_No',
            'vpf.Product_Desc as product_name',
            'vpf.Flag',
            DB::raw("SUM(CASE WHEN vpf.Date BETWEEN '$fromDate' AND '$toDate' THEN vpf.Prodcut_Qty ELSE 0 END) as curr_qty"),
            DB::raw("SUM(CASE WHEN vpf.Date BETWEEN '$fromDate' AND '$toDate' THEN vpf.Prodcut_Qty * vpf.Prodcut_Price ELSE 0 END) as curr_total"),
            DB::raw("SUM(CASE WHEN vpf.Date BETWEEN DATE_SUB('$fromDate', INTERVAL 1 YEAR) AND DATE_SUB('$toDate', INTERVAL 1 YEAR) THEN vpf.Prodcut_Qty ELSE 0 END) as prev_qty"),
            DB::raw("SUM(CASE WHEN vpf.Date BETWEEN DATE_SUB('$fromDate', INTERVAL 1 YEAR) AND DATE_SUB('$toDate', INTERVAL 1 YEAR) THEN vpf.Prodcut_Qty * vpf.Prodcut_Price ELSE 0 END) as prev_total")
        );

    if ($supRef) {
        $productBaseQuery->where('vpf.Sup_Ref', $supRef);
    }

    if ($flag) {
        $productBaseQuery->where('vpf.Flag', $flag);
    }

    if ($productSearch) {
        $productBaseQuery->where(function ($q) use ($productSearch) {
            $q->where('vpf.Model_No', 'like', "%$productSearch%")
              ->orWhere('vpf.Product_Desc', 'like', "%$productSearch%");
        });
    }

    $productsRaw = $productBaseQuery->groupBy('vpf.pro_id')->get();
    $prevFYLabel = ($fromYear - 1) . '-' . ($toYear - 1);

    $productsShaped = $productsRaw->map(function ($row) use ($financialYear, $prevFYLabel, $currencyType, $currencyRates, $domesticCode) {
        $currTotal = (float) $row->curr_total;
        $prevTotal = (float) $row->prev_total;
        $currPriceUnit = $currTotal && $row->curr_qty ? ($currTotal / $row->curr_qty) : 0;
        $prevPriceUnit = $prevTotal && $row->prev_qty ? ($prevTotal / $row->prev_qty) : 0;

        $flag = $row->Flag ?? null;
        $multiplier = 1;
        if ($currencyType === 'default_currency' && $flag && isset($currencyRates[$flag])) {
            $multiplier = $currencyRates[$flag];
        }

        $currTotal *= $multiplier;
        $prevTotal *= $multiplier;
        $currPriceUnit *= $multiplier;
        $prevPriceUnit *= $multiplier;
        ...
        return [
            ...
            'previous_fy'     => $prevFYLabel,
            'prev_qty'        => (int) $row->prev_qty,
            'prev_price_unit' => round($prevPriceUnit, 2),
            'prev_total'      => round($prevTotal, 2),
            'current_fy'      => $financialYear,
            'curr_qty'        => (int) $row->curr_qty,
            'curr_price_unit' => round($currPriceUnit, 2),
    ...
    $yearOnYear = [];
    $currencyCode = $domesticCode;
    $currencySymbol = $currencySymbolsHtml[$domesticCode] ?? '';

    for ($i = $fromYear - 4; $i <= $toYear; $i++) {
        $start = "$i-04-01";
        $end = ($i + 1) . "-03-31";

        // --- PO Total ---
        $entries = DB::table('vendor_po_final as vpf')
            ->join('vendor_po_order as vpo', 'vpf.PO_ID', '=', 'vpo.ID')
            ->whereBetween('vpf.Date', [$start, $end]);

        if ($supRef) $entries->where('vpf.Sup_Ref', $supRef);
        if ($flag)   $entries->where('vpf.Flag', $flag);

        $entries = $entries->select(
            DB::raw('SUM(vpf.Prodcut_Qty * vpf.Prodcut_Price) as total'),
            'vpf.Flag'
        )->groupBy('vpf.Flag')->get();
        ...
        foreach ($entries as $entry) {
            $code = $entry->Flag;
            $multiplier = 1;
            if ($currencyType === 'default_currency' && $code && isset($currencyRates[$code])) {
                $multiplier = $currencyRates[$code];
            }
            $total += $entry->total * $multiplier;
        }

        if ($currencyType === 'import_currency' && $code) {
            $currencyCode = $code;
            $currencySymbol = $currencySymbolsHtml[$code] ?? '';
        }

        // --- Invoice Total ---
        $invoiceEntries = DB::table('vendor_po_invoice_new as inv')
            ->join('vendor_po_order as po', 'inv.po_id', '=', 'po.ID')
            ->whereBetween('inv.invoice_date', [$start, $end]);

        if ($supRef) $invoiceEntries->where('po.VPI', $supRef);
        if ($flag)   $invoiceEntries->where('po.Flag', $flag);

        $invoiceEntries = $invoiceEntries->select(
            DB::raw('SUM(inv.value) as total_invoice'),
            'po.Flag'
        )->groupBy('po.Flag')->get();
        ...
        foreach ($invoiceEntries as $inv) {
            $invFlag = $inv->Flag;
            $multiplier = 1;
            if ($currencyType === 'default_currency' && $invFlag && isset($currencyRates[$invFlag])) {
                $multiplier = $currencyRates[$invFlag];
            }
            $invoiceTotal += $inv->total_invoice * $multiplier;
        }
        ...
        $yearOnYear[] = [
            ...
            'currency_symbol'   => $currencySymbol,
            'current_year'      => round($total, 2),
            'previous_year'     => null,
            'growth_percentage' => null,
            'invoice_amount'    => round($invoiceTotal, 2),
        ];
    }

    // Calculate growth %
    for ($i = 1; $i < count($yearOnYear); $i++) {
        $prev = $yearOnYear[$i - 1]['current_year'];
        $curr = $yearOnYear[$i]['current_year'];
        $yearOnYear[$i]['previous_year'] = round($prev, 2);
        $yearOnYear[$i]['growth_percentage'] = $prev > 0 ? round((($curr - $prev) / $prev) * 100, 2) : null;
    }

    $currencyWiseTotals = [];
    if ($currencyType === 'import_currency') {
        $currencyWiseTotals[] = [
            'currency_code'   => $currencyCode,
            'currency_symbol' => $currencySymbol,
            'amount'          => $yearOnYear[count($yearOnYear) - 1]['current_year'],
        ];
    } else {
        $currencyWiseTotals[] = [
            'currency_code'   => $domesticCode,
            'currency_symbol' => $currencySymbolsHtml[$domesticCode] ?? '',
            'amount'          => $yearOnYear[count($yearOnYear) - 1]['current_year'],
        ];
    }
    ...
    return response()->json([
        ...
        'year_on_year'         => $yearOnYear,
        'currencyWiseTotals'   => $currencyWiseTotals,
        'products'             => $productsShaped,
        'pagination'           => [
            'pageno'        => 1,
            'records'       => 10,
            'total_records' => count($productsShaped),
            'total_pages'   => 1,
        ]
    ]);
}
...
public function purchaseTrendsYOY_rumitok(Request $request)
{
    $financialYear = $request->financial_year;
    $supRef        = $request->sup_ref;
    $flag          = $request->flag;
...
// Default to domestic_currency if not specified
if (!$currencyType) {
    $currencyType = 'default_currency';
}
    $fy = explode('-', $financialYear);
    ...
    $currencyRates = DB::table('tbl_currencies')
        ->where('deleteflag', 'active')
        ->pluck('currency_value', 'currency_code')->toArray();

    $currencySymbolsHtml = DB::table('tbl_currencies')->where('deleteflag', 'active')->pluck('currency_html_code', 'currency_code')->toArray();

    $productBaseQuery = DB::table('vendor_po_final as vpf')
        ->join('vendor_po_order as vpo', 'vpf.PO_ID', '=', 'vpo.ID')
      //  ->join('tbl_products as p', 'vpf.Product_ID', '=', 'p.ID')
        ->select(
            DB::raw("DATE_FORMAT(vpf.Date, '%Y-%m-%d') as date"),
            'vpf.PO_ID as po_id',
            'vpf.pro_id as pro_id',
    ...
    if ($productSearch) {
        $productBaseQuery->where(function ($q) use ($productSearch) {
            $q->where('p.Model_No', 'like', "%$productSearch%")
              ->orWhere('p.Product_Name', 'like', "%$productSearch%");
        });
    }
    ...
    $productsShaped = $productsRaw->map(function ($row) use ($financialYear, $prevFYLabel, $currencyType, $currencyRates, $domesticCode) {
        ...
        // Currency conversion for default_currency
        $flag = $row->Flag ?? null;
        $multiplier = 1;
        if ($currencyType === 'default_currency' && $flag && isset($currencyRates[$flag])) {
            $multiplier = $currencyRates[$flag];
    ...
    for ($i = $fromYear - 4; $i <= $toYear; $i++) {
        ...
        if ($supRef) {
            $entries->where('vpf.Sup_Ref', $supRef);
        }

        if ($flag) {
            $entries->where('vpf.Flag', $flag);
        }
    ...
    for ($i = 1; $i < count($yearOnYear); $i++) {
        $prev = $yearOnYear[$i - 1]['current_year'];
        $curr = $yearOnYear[$i]['current_year'];
        $yearOnYear[$i]['previous_year'] = $prev;
        $yearOnYear[$i]['growth_percentage'] = $prev > 0 ? round((($curr - $prev) / $prev) * 100, 2) : null;
    }

    // Currency wise totals (for chart)
    $currencyWiseTotals = [];
    if ($currencyType === 'import_currency') {
        $currencyWiseTotals[] = [
            'currency_code'   => $currencyCode,
...
public function purchaseTrendsYOY_old(Request $request)
{
    /* -------------------------------------------------------------------- *
     * 1 REQUEST PARAMS & CORE DATES
     * -------------------------------------------------------------------- */
 ...
 $productsBaseQuery = DB::table('vendor_po_final as vpf')
    ...
    $topProducts      = $productsSorted->slice($offset, $records)->values();
// --------------------------------------------------------------------
// 6 CURRENCY-WISE TOTALS (ALL-TIME or based on FY range)
// --------------------------------------------------------------------
$currencyRaw = DB::table('vendor_po_final as vpf')
    ->whereBetween('Date', [$currFYStart, $currFYEnd])
    ->when($supRef, fn ($q) => $q->where('Sup_Ref', $supRef))
    ->select(
        'Flag as currency_code',
        DB::raw('SUM(Prodcut_Qty * Prodcut_Price) as amount')
    )
    ->whereNotNull('Flag')
    ->groupBy('Flag')
    ->get();
...
//MOM
public function purchaseTrendsMOM(Request $request)
{
    /* -------------------------------------------------------------------- *
     * 1 REQUEST PARAMS & CORE DATES
     * -------------------------------------------------------------------- */
    $supRef        = $request->input('sup_ref');
    $flag          = trim($request->input('flag')) ?: null;
    $productSearch = trim($request->input('product_search') ?: $request->input('searchBy'));

    $pageno  = (int) $request->input('pageno', 1);
    $records = (int) $request->input('records', 10);
    $offset  = ($pageno - 1) * $records;
/*
    $currFyYear = trim($request->input('curr_fy_year'));
    $prevFyYear = trim($request->input('prev_fy_year'));*/
	
	
	
	
	$financialYear = trim($request->input('financial_year'));
    $currFyYear    = $financialYear !== '' ? $financialYear : trim($request->input('curr_fy_year'));
    $prevFyYear    = trim($request->input('prev_fy_year'));

    $now = Carbon::now();
    $defaultCurrYear = $now->month >= 4 ? $now->year : $now->year - 1;

    $currStartYear = $currFyYear ? (int) explode('-', $currFyYear)[0] : $defaultCurrYear;
    $prevStartYear = $prevFyYear ? (int) explode('-', $prevFyYear)[0] : ($currStartYear - 1);

    $currFYStart = Carbon::createFromDate($currStartYear, 4, 1)->startOfDay();
    $currFYEnd   = Carbon::createFromDate($currStartYear + 1, 3, 31)->endOfDay();
    $prevFYStart = Carbon::createFromDate($prevStartYear, 4, 1)->startOfDay();
    $prevFYEnd   = Carbon::createFromDate($prevStartYear + 1, 3, 31)->endOfDay();

    $financialYear = $currStartYear . '-' . ($currStartYear + 1);
    $prevFYLabel   = $prevStartYear . '-' . ($prevStartYear + 1);

    /* -------------------------------------------------------------------- *
     * 2 CURRENCY CODE & SYMBOL
     * -------------------------------------------------------------------- */
    $firstCurrencyRow = DB::table('vendor_po_final')
        ->whereBetween('Date', [$currFYStart, $currFYEnd])
        ->when($supRef, fn ($q) => $q->where('Sup_Ref', $supRef))
        ->when(!is_null($flag), fn ($q) => $q->where('Flag', $flag))
        ->whereNotNull('Flag')
    ...
    $currencyCode   = $firstCurrencyRow->Flag ?? ($flag ?? 'ALL');
    $symbolData     = currencySymbolByCurrencyCode($currencyCode);
    $currencySymbol = $symbolData[0] ?? '';

    /* -------------------------------------------------------------------- *
     * 3 SHARED QUERY CLOSURE
     * -------------------------------------------------------------------- */
    $makeQuery = function ($from, $to) use ($supRef, $flag) {
        return DB::table('vendor_po_final')
            ->whereBetween('Date', [$from, $to])
    ...
    return response()->json([
        'filters' => [
            'curr_fy_year'    => $currStartYear . '-' . ($currStartYear + 1),
            'prev_fy_year'    => $prevStartYear . '-' . ($prevStartYear + 1),
            'from_date'       => $currFYStart->format('Y-m-d'),
            'to_date'         => $currFYEnd->format('Y-m-d'),
            'sup_ref'         => $supRef,
            'flag'            => $flag,
...
public function purchaseTrendsMOM_old_31_07_2025(Request $request)
{
    /* -------------------------------------------------------------------- *
     * 1 REQUEST PARAMS & CORE DATES
     * -------------------------------------------------------------------- */
    ...
    for ($m = 0; $m < 12; $m++) {
        ...
        $mom->push([
            ...
            'currency_symbol' => $currencySymbol,  //  added
        ]);
    }
...
public function purchaseTrendsQOQ(Request $request)
{
    /* -------------------------------------------------------------------- *
     * 1 REQUEST PARAMS & CORE DATES
     * -------------------------------------------------------------------- */
    ...
    if ($financialYear) {
        ...
        $now          = now();
        $fyStartYear  = $now->month < 4 ? $now->year - 1 : $now->year;
        $fyEndYear    = $fyStartYear + 1;
        $financialYear = "$fyStartYear-$fyEndYear";
    }

    $currFYStart  = Carbon::create($fyStartYear, 4, 1)->startOfDay();
    $currFYEnd    = Carbon::create($fyEndYear, 3, 31)->endOfDay();
    $prevFYStart  = $currFYStart->copy()->subYear();
    $prevFYEnd    = $currFYEnd->copy()->subYear();
    $prevFYLabel  = ($fyStartYear - 1) . '-' . ($fyEndYear - 1);

    /* -------------------------------------------------------------------- *
     * 2 BASE QUERY BUILDER FUNCTION
     * -------------------------------------------------------------------- */
    $makeQuery = function ($from, $to) use ($supRef, $flag) {
        return DB::table('vendor_po_final')
            ->whereBetween('Date', [$from, $to])
    ...
    /* -------------------------------------------------------------------- *
     * 3 QUARTER DEFINITIONS
     * -------------------------------------------------------------------- */
    $quarters = [
        'Q1' => [4, 6],   // Apr Jun
        'Q2' => [7, 9],   // Jul Sep
        'Q3' => [10, 12], // Oct Dec
        'Q4' => [1, 3],   // Jan Mar (of next calendar year)
    ];

    /* -------------------------------------------------------------------- *
     * 4 GET DEFAULT CURRENCY CODE & SYMBOL (GBP, , etc.)
     * -------------------------------------------------------------------- */
    $firstCurrencyRow = DB::table('vendor_po_final')
        ->whereBetween('Date', [$currFYStart, $currFYEnd])
        ->when($supRef, fn ($q) => $q->where('Sup_Ref', $supRef))
    ...
    $currencyCode   = $firstCurrencyRow->Flag ?? ($flag ?? 'ALL');
    $currencySymbol = currencySymbolByCurrencyCode($currencyCode)[2] ?? '';

    /* -------------------------------------------------------------------- *
     * 5 BUILD QUARTER-ON-QUARTER COMPARISON
     * -------------------------------------------------------------------- */
    $qoq = collect();
    ...
    foreach ($quarters as $label => [$startMonth, $endMonth]) {
        // Current FY quarter
        $currStart = Carbon::create(
            $startMonth < 4 ? $currFYStart->year + 1 : $currFYStart->year,
            $startMonth, 1
        )->startOfDay();

        $currEnd = Carbon::create(
            $endMonth < 4 ? $currFYStart->year + 1 : $currFYStart->year,
            $endMonth, 1
        )->endOfMonth()->endOfDay();
...
public function purchaseTrendsTopProducts(Request $request)
{
    /* -------------------------------------------------------------------- *
     * 1 REQUEST PARAMS & CORE DATES
     * -------------------------------------------------------------------- */
    ...
    $productsShaped = $productsRaw->map(function ($row) use ($financialYear, $prevFYLabel) {
        ...
        $prevTotal = (float) $row->prev_total;
        $pct       = $prevTotal > 0 ? round((($currTotal - $prevTotal) / $prevTotal) * 100, 2) : null;
	 $currency = $row->Flag; //exit;
	$currency_name = currencySymbolByCurrencyCode($currency);

//	print_r($currency_name);
	 //$currency_symbol = $currency_name[2] ?: 'N/A'; //currencySymbolByCurrencyCode($row->Flag);
        return [
          /*  'pro_id'          => $row->pro_id,
            'product_name'    => $row->product_name,*/
			'po_id'           => $row->po_id,
...
//			'currency_symbol' => $currency_name[2] ?: 'N/A',
			'currency_symbol' => isset($currency_name[2]) ? $currency_name[2] : 'N/A',
            'previous_fy'     => $prevFYLabel,
            'prev_qty'        => (int)    $row->prev_qty,
            'prev_price_unit' => (float)  $row->prev_price_unit,
            'prev_total'      => $prevTotal,
...
public function updatePOApprovalStatus(Request $request)
{
    $request->validate([
    ...
    // Check if PO exists
    $po = DB::table('vendor_po_order')->where('ID', $poId)->first();
    if (!$po) {
        return response()->json(['status' => false, 'message' => 'PO not found.'], 404);
    }

    $updateData = [];

    if ($action === 'approve' || $action === 0 || $action === '0') {
        $updateData = [
            'po_type' => 0,
            'Confirm_Purchase' => 'active',
            'reject_remark' => null,
        ];
    } elseif ($action === 'reject' || $action === 3 || $action === '3') {
        if (empty($remark)) {
            return response()->json(['status' => false, 'message' => 'Remark is required for rejection.'], 422);
        }

        $updateData = [
            'po_type' => 3,
            'Confirm_Purchase' => 'rejected',
    ...
    DB::table('vendor_po_order')->where('ID', $poId)->update($updateData);
...
public function getPurchaseOrderTypeCounts(Request $request)
{
    $fy = $request->input('financial_year');

    // Get current FY if not passed
    if (!$fy) {
    ...
    // FY date range
    $fromDate = \Carbon\Carbon::createFromFormat('Y-m-d', explode('-', $fy)[0] . '-04-01')->startOfDay();
    $toDate   = \Carbon\Carbon::createFromFormat('Y-m-d', explode('-', $fy)[1] . '-03-31')->endOfDay();

    // Import count and amount (converted to INR)
    $importData = DB::table('vendor_po_order as o')
        ->join('vendor_master as v', 'o.VPI', '=', 'v.ID')
        ->join('vendor_po_final as f', 'f.PO_ID', '=', 'o.ID')
        ->leftJoin('tbl_currencies as c', 'v.Currency', '=', 'c.currency_code')
        ->where('v.purchase_type', 'import')
        ->where('o.Confirm_Purchase', 'active')
        ->whereBetween('f.Date', [$fromDate, $toDate])
        ->selectRaw('
            COUNT(DISTINCT o.ID) as count,
            SUM(f.Prodcut_Qty * f.Prodcut_Price * IFNULL(c.currency_value, 1)) as amount_inr
        ')
        ->first();

    // Domestic count and amount (already in INR or treated as INR)
    $domesticData = DB::table('vendor_po_order as o')
        ->join('vendor_master as v', 'o.VPI', '=', 'v.ID')
        ->join('vendor_po_final as f', 'f.PO_ID', '=', 'o.ID')
        ->leftJoin('tbl_currencies as c', 'v.Currency', '=', 'c.currency_code')
        ->where('v.purchase_type', 'domestic')
        ->where('o.Confirm_Purchase', 'active')
        ->whereBetween('f.Date', [$fromDate, $toDate])
        ->selectRaw('
            COUNT(DISTINCT o.ID) as count,
    ...
    return response()->json([
        ...
        'import_amount_inr' => round((float) $importData->amount_inr, 2),
        'domestic_count' => (int) $domesticData->count,
        'domestic_amount_inr' => round((float) $domesticData->amount_inr, 2)
    ]);
}


public function getPurchaseOrderTypeCountryWise(Request $request)
{
    $fy = $request->input('financial_year');
    ...
    // Get distinct country names from vendor_master that are used in confirmed POs
    $countries = DB::table('vendor_master as v')
        ->join('vendor_po_order as o', 'v.ID', '=', 'o.VPI')
        ->join('vendor_po_final as f', 'f.PO_ID', '=', 'o.ID')
        ->where('o.Confirm_Purchase', 'active')
        ->whereBetween('f.Date', [$fromDate, $toDate])
    ...
    foreach ($countries as $countryName) {
        // Get country meta (lat/lon)
        $countryMeta = DB::table('tbl_country')
        ...
        if (!$countryMeta) {
            continue; // skip if no matching country info
        }

        // Import
        $import = DB::table('vendor_po_order as o')
            ->join('vendor_master as v', 'o.VPI', '=', 'v.ID')
            ->join('vendor_po_final as f', 'f.PO_ID', '=', 'o.ID')
            ->leftJoin('tbl_currencies as c', 'v.Currency', '=', 'c.currency_code')
            ->where('o.Confirm_Purchase', 'active')
            ->where('v.purchase_type', 'import')
            ->where('v.Country', $countryName)
            ->whereBetween('f.Date', [$fromDate, $toDate])
            ->selectRaw('COUNT(DISTINCT o.ID) as count, SUM(f.Prodcut_Qty * f.Prodcut_Price * IFNULL(c.currency_value, 1)) as amount_inr')
            ->first();

        // Domestic
        $domestic = DB::table('vendor_po_order as o')
            ->join('vendor_master as v', 'o.VPI', '=', 'v.ID')
            ->join('vendor_po_final as f', 'f.PO_ID', '=', 'o.ID')
            ->leftJoin('tbl_currencies as c', 'v.Currency', '=', 'c.currency_code')
            ->where('o.Confirm_Purchase', 'active')
       ...
       $results[] = [
    'location' => [
        (float) $countryMeta->latitude,
        (float) $countryMeta->longitude
    ],
    'country' => $countryMeta->country_name,
    'import_count' => (int) ($import->count ?? 0),
    'import_amount_inr' => round((float) ($import->amount_inr ?? 0), 2),
    'domestic_count' => (int) ($domestic->count ?? 0),
    'domestic_amount_inr' => round((float) ($domestic->amount_inr ?? 0), 2),
];
    ...
    return response()->json([
        'fy' => $fy,
        'country_wise_data' => $results
    ]);
}



public function getProductDetailsByProId_old_5_aug_2025(Request $request)
{
    $pro_id = $request->input('pro_id');
    $currencyType = $request->input('currency_type', 'default_currency');
    ...
    $product = DB::table('tbl_products as tp')
        ->leftJoin('tbl_products_entry as tpe', 'tp.pro_id', '=', 'tpe.pro_id')
        ->leftJoin('tbl_application as ta', 'tpe.app_cat_id', '=', 'ta.application_id')
        ->where('tp.pro_id', $pro_id)
        ->select(
            'tp.pro_id',
            'tp.pro_title',
            'tp.admin_moq as recommended_quantity_mo',
    ...
    $invoiceQtyMap = getTotalQtyInvoiceBulk($fyStart->toDateString(), $fyEnd->toDateString(), [$pro_id]);
    $poSummaryMap = getPoSummariesBulk([$pro_id], $fyStart, $fyEnd, $lastFyStart, $lastFyEnd, $currencyType);
    $poHistoryMap = getPoHistoriesBulk([$pro_id], $currencyType);

    $invoicedQty = $invoiceQtyMap[$pro_id] ?? 0;
    $summary = $poSummaryMap[$pro_id] ?? [];

    $rawYtdPrice = $summary['ytd']['avg_price'] ?? 0;
    $rawLastFyPrice = $summary['last_fy']['avg_price'] ?? 0;
    ...
    $firstVendorId = $poHistoryMap[$pro_id][0]['vendor_id'] ?? null;
    if ($firstVendorId) {
        $vendorData = DB::table('vendor_master')->where('id', $firstVendorId)->select('Currency')->first();
        if ($vendorData && $vendorData->Currency) {
            $currency = $vendorData->Currency;

            $symbolData = DB::table('tbl_currencies')
                ->where('currency_code', $currency)
                ->select('currency_symbol', 'currency_value')
                ->first();

            if ($symbolData) {
                $symbol = $symbolData->currency_symbol;
                $currencyValue = $symbolData->currency_value ?? 1;

                if ($currencyType === 'import_currency') {
                    $rawYtdPrice = ($summary['ytd']['avg_price'] ?? 0) * $currencyValue;
                    $rawLastFyPrice = ($summary['last_fy']['avg_price'] ?? 0) * $currencyValue;
                }
            }
        }
    }

    $priceValueYTD = ($rawLastFyPrice == 0 && $rawYtdPrice > 0) ? 100 :
        (($rawLastFyPrice == 0 && $rawYtdPrice == 0) ? 0 :
            round((($rawYtdPrice - $rawLastFyPrice) / $rawLastFyPrice) * 100, 2));
    $priceChangeYTD = $priceValueYTD > 0 ? "+{$priceValueYTD}%" : ($priceValueYTD < 0 ? "{$priceValueYTD}%" : '0%');
    $absoluteDiffYTD = $rawYtdPrice - $rawLastFyPrice;

    $product->invoiced_total_quantity_of_product = (int) $invoicedQty;
    $product->ytd = [
        'label' => 'YTD (' . $currentFyLabel . ')',
        'purchase_quantity' => $summary['ytd']['purchase_quantity'] ?? 0,
        'avg_price' => round($rawYtdPrice, 2),
        'price_change_vs_last_fy' => $priceChangeYTD,
        'price_change_value' => $priceValueYTD,
        'price_diff_vs_last_fy' => round($absoluteDiffYTD, 2),
        'currency_symbol' => $symbol
    ];
    $product->last_fy = [
        'label' => 'Last FY (' . $lastFyLabel . ')',
        'purchase_quantity' => $summary['last_fy']['purchase_quantity'] ?? 0,
        'avg_price' => round($rawLastFyPrice, 2),
        'currency_symbol' => $symbol
    ];
    ...
    if (!empty($poHistoryMap[$pro_id])) {
//        foreach ($poHistoryMap[$pro_id] as $row) {
			foreach ($poHistoryMap[$pro_id] ?? [] as $row) {
            $rowCurrencyValue = $currencyValue;
            $convertedPrice = ($currencyType === 'import_currency') ? $row['price'] * $rowCurrencyValue : $row['price'];

            $product->po_history[] = [
                'po_date' => $row['Date'],
                'default_price' => $row['price'],
                'currencyValue' => $rowCurrencyValue,
                'Currency' => $currency,
                'vendor_Currency' => $vendorData->Currency ?? null,
                'Currency_Symbol' => $symbol,
            ...
            if (!$vendorId && !empty($row['vendor_id'])) {
                $vendorId = $row['vendor_id'];
            }
        }

        $lastPO = $product->po_history[0];
        $product->po_date = $lastPO['po_date'] ?? null;
        $product->Currency = $currency;
        $product->vendor_name = $lastPO['vendor_name'] ?? null;
        $product->last_purchase_price = $lastPO['price'] ?? null;
        $product->last_purchase_quantity = $lastPO['quantity'] ?? null;
    } else {
        $product->Currency = $currency;
    }
...
public function getProductDetailsByProId(Request $request)
{
    $pro_id = $request->input('pro_id');
    $currencyType = $request->input('currency_type', 'default_currency');
    ...
    $symbol = $defaultCurrencySymbol;
    $currencyValue = 1;
    $vendorCurrency = $defaultCurrencyCode;
...
// Only use vendor currency if type is import_currency
if ($currencyType === 'import_currency') {
    $firstVendorId = $poHistoryMap[$pro_id][0]['vendor_id'] ?? null;
    ...
    if ($firstVendorId) {
        ...
        if ($vendorData && $vendorData->Currency) {
            $vendorCurrency = $vendorData->Currency;

            $symbolData = DB::table('tbl_currencies')
                ->where('currency_code', $vendorCurrency)
                ->select('currency_code','currency_symbol','currency_css_symbol', 'currency_value')
                ->first();

            if ($symbolData) {
                $currency = $vendorCurrency;
                $symbol = $symbolData->currency_symbol ?? $defaultCurrencySymbol;
                $currencyValue = $symbolData->currency_value ?? 1;
            }
        }
    }
 ...
 /*   $product->ytd = [
        ...
        'purchase_quantity' => $summary['ytd']['purchase_quantity'] ?? 0,
        'avg_price_in_vendor_currency' => round($summary['ytd']['avg_price'] ?? 0, 2),
        'avg_price_in_inr' => round(($summary['ytd']['avg_price'] ?? 0) * $currencyValue, 2),
        'price_change_vs_last_fy' => $priceChangeYTD,
        'price_change_value' => $priceValueYTD,
		'avg_price' => $priceValueYTD,
    ...
    $product->last_fy = [
		...
		'currencyValue' => $currencyValue,
        'purchase_quantity' => $summary['last_fy']['purchase_quantity'] ?? 0,
        'avg_price_in_vendor_currency' => round($summary['last_fy']['avg_price'] ?? 0, 2),
        'avg_price_in_inr' => round(($summary['last_fy']['avg_price'] ?? 0) * $currencyValue, 2),
		'avg_price' => round($summary['last_fy']['avg_price'] ?? 0, 2),
        'currency_symbol' => $symbol
    ];
...
$vendorCurrencyCode   = $summary['vendor_currency_code'] ?? '';  // From DB or context
$vendorCurrencySymbol = $summary['vendor_currency_symbol'] ?? ''; // From DB or context
$currencyValue        = $summary['vendor_currency_value'] ?? 1;
//print_r($summary);
// Determine symbols based on currency type
if ($currencyType === 'default_currency') {
    $symbol = $defaultCurrencySymbol;
} elseif ($currencyType === 'import_currency') {
    $symbol = $vendorCurrencySymbol;
} else {
    $symbol = ''; // fallback
}

$priceValueLastFY = round(($summary['last_fy']['avg_price'] ?? 0) * $currencyValue, 2);




       $ytdPrice = round(($summary['ytd']['avg_price'] ?? 0) * $currencyValue, 2);
$lastFyPrice = round(($summary['last_fy']['avg_price'] ?? 0) * $currencyValue, 2);

// Price change for Last FY compared to YTD (reverse of what you show for YTD block)
$priceValueLastFY = ($ytdPrice == 0 && $lastFyPrice > 0) ? 100 :
                    (($ytdPrice == 0 && $lastFyPrice == 0) ? 0 :
                    round((($lastFyPrice - $ytdPrice) / $ytdPrice) * 100, 2));
...
// Optional: absolute price difference
$absoluteDiffYTD = $ytdPrice - $lastFyPrice;
$absoluteDiffLastFY = $lastFyPrice - $ytdPrice;
...
$product->ytd = [
    ...
    'purchase_quantity' => $summary['ytd']['purchase_quantity'] ?? 0,
    'avg_price_in_vendor_currency' => round($summary['ytd']['avg_price'] ?? 0, 2),
    'avg_price_in_inr' => round(($summary['ytd']['avg_price'] ?? 0), 2),
    'price_change_vs_last_fy' => $priceChangeYTD,
    'price_change_value' => $priceValueYTD,
    'avg_price_old' => $priceValueYTD,
	'avg_price' => round(($summary['ytd']['avg_price'] ?? 0) , 2),

    'price_diff_vs_last_fy' => round($absoluteDiffYTD, 2),
    'currency_symbol' => $symbol,
    'vendor_currency_symbol' => $vendorCurrencySymbol,
...
$product->last_fy = [
    ...
    'purchase_quantity' => $summary['last_fy']['purchase_quantity'] ?? 0,
    'avg_price_in_vendor_currency' => round($summary['last_fy']['avg_price'] ?? 0, 2),
    'avg_price_in_inr' => round(($summary['last_fy']['avg_price'] ?? 0), 2),
    'avg_price' => round(($summary['last_fy']['avg_price'] ?? 0) , 2),
    'price_change_vs_current' => $priceChangeYTD, // YTD vs Last FY
    'price_change_value' => $priceValueLastFY,     // Last FY vs YTD
    'price_diff_vs_last_fy' => $absoluteDiffLastFY, // rupeeer difference
    'currency_symbol' => $symbol,
    'vendor_currency_symbol' => $vendorCurrencySymbol,
    'default_currency_symbol' => $defaultCurrencySymbol,
];
    ...
    $product->po_history = [];
    $totalPoValueVendor = 0;
    $totalPoValueInr = 0;

  //  foreach ($poHistoryMap[$pro_id] as $row) {
		foreach ($poHistoryMap[$pro_id] ?? [] as $row) {
//        $priceVendor = $row['price'];
//       // $priceInr = $priceVendor * $currencyValue;
//	   $priceVendor = $row['price'];
...
if ($currencyType === 'default_currency') {
    // Convert from INR to vendor currency for display if needed
    $priceVendor = $row['price'] / $currencyValue;
}

$priceInr = $priceVendor * $currencyValue;

// Ensure priceVendor is always in vendor currency
/*if ($currencyType === 'default_currency') {
    $priceVendor = $row['price'] / $currencyValue;
}
*/
//$priceInr = $priceVendor * $currencyValue;
        ...
        $product->po_history[] = [
            'po_date' => $row['Date'],
			'PO_ID' => $row['PO_ID'],
            'price_in_vendor_currency' => round($priceVendor, 2),
            'price_in_inr' => round($priceInr, 2),
		    'currencyValue' => $currencyValue,
            'price' => $row['price'],
			'quantity' => $quantity,
            'vendor_name' => $row['vendor_name'],
        ...
        $totalPoValueVendor += $priceVendor * $quantity;
        $totalPoValueInr += $priceInr * $quantity;
    }

    $product->total_po_value_in_vendor_currency = round($totalPoValueVendor, 2);
    $product->total_po_value_in_inr = round($totalPoValueInr, 2);
/*    $product->Currency = $vendorCurrency;
    $product->Currency_Symbol = $symbol;*/
	
$get_currency_symbol=currencySymbolByCurrencyCode($currency);
	$product->currency_type = $currencyType;
	$product->currency_code = $currency;
	$product->default_currency_code = $defaultCurrencyCode;
	$product->import_currency_code = $vendorCurrency;
	
	$product->default_currency_symbol = $defaultCurrencySymbol;
	$product->import_currency_symbol = $symbol;
	
	$product->Currency = $currency; // Final used currency
	$product->Currency_Symbol = $get_currency_symbol[2];// currencySymbolByCurrencyCode($currency);//$symbol; // Final used symbol

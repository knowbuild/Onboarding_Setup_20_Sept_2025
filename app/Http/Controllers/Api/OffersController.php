...
use App\Models\InvoiceDeliveryMethod;
...
//edit lead
 ...
 $offer_probability     =$lead_array["offer_probability"];
...
//	$dataOrderArray["payment_mode"] 			= $PaymentMode;
	...
	if($offer_probability!='' && $offer_probability=='0')
	{
	$dataOrderArray["offer_probability"]		= 	$offer_probability;
	}
	else
	{
	$dataOrderArray["offer_probability"]		= "3"; //default enq/order status after creating offer i.e. 50%		
...
$namespace="f3bd3f7d_bde9_45cb_9083_9ce1b465c6d8";

$bodyVars = [
    (string) ($acc_manager_name_finance ?? ''),   // {{1}}
    (string) ($pcode                 ?? ''),      // {{2}}
    (string) ($billing_company       ?? ''),      // {{3}}
    (string) (admin_name($Prepared_by) ?? '')     // {{4}}
];
$response1 = whatsapp_template(
    $acc_manager_phone_finance,          // destination
    'proforma_invoice_approval_sent',    // template name
	$bodyVars,
    $namespace,
    '86wkjz6mc9kyttpp',                  // token
    '85376'                              // instance / channel ID
);
...
$response2 = whatsapp_template(
    '919811169723',          // destination
    'proforma_invoice_approval_sent',    // template name
	$bodyVars,
    $namespace,
    '86wkjz6mc9kyttpp',                  // token
...
$bodyVars = [
    (string) ($acc_manager_name_finance_sanjeev ?? ''),   // {{1}}
    (string) ($pcode                 ?? ''),      // {{2}}
    (string) ($billing_company       ?? ''),      // {{3}}
    (string) (admin_name($Prepared_by) ?? '')     // {{4}}
];
$response3 = whatsapp_template(
    $acc_manager_phone_finance_sanjeev,          // destination
    'proforma_invoice_approval_sent',    // template name
	$bodyVars,
    $namespace,
    '86wkjz6mc9kyttpp',                  // token
...
$bodyVars = [
    (string) ($acc_manager_name_finance_sha ?? ''),   // {{1}}
    (string) ($pcode                 ?? ''),      // {{2}}
    (string) ($billing_company       ?? ''),      // {{3}}
    (string) (admin_name($Prepared_by) ?? '')     // {{4}}
];
$response3 = whatsapp_template(
    $acc_manager_phone_finance_sha,          // destination
    'proforma_invoice_approval_sent',    // template name
	$bodyVars,
    $namespace,
    '86wkjz6mc9kyttpp',                  // token
...
/*Mail::html($html, function ($message) use ($toEmails, $ccEmails, $invoiceNo) {
    $message
	    ->from('sales@stanlay.com', 'Knowbuild CRM')
        ->to($toEmails)
        ->cc($ccEmails)
...
// echo $orders_id; die();
 ...
 if($orders_id) {
  ...
  ->where('O_Id', $orderNO)->with(['paymentTerms', 'accountManager','modeMaster','warrantyMaster'])->first();
...
LEFT JOIN tbl_web_enq_edit  ON o.edited_enq_id = tbl_web_enq_edit.ID 
where 
...
public function company_directory_listing(Request $request)
{
    try {
        $validated = $request->validate([
            'acc_manager' => 'nullable|string', // allow comma-separated or single number
            'month' => 'nullable',
            ...
            'admin_role_id' => 'nullable|in:0,1,2,3,4,5',
            'currentuserid' => 'nullable|numeric',
            'pcode' => 'nullable',
    ...
    // Parse acc_manager into array of integers
    $accManager = collect(explode(',', (string) $request->acc_manager))
        ->map(fn($id) => (int) trim($id))
        ->filter(fn($id) => $id > 0)
        ->values()
        ->toArray();

    $query = DB::table('tbl_comp')
        ->where('deleteflag', 'active')
        ->where('india_mart_co', 'no');

    // Filter: Account Manager
   /* if (!empty($accManager)) {
        $query->whereIn('acc_manager', $accManager);
    }*/
    ...
    $sortOptions = [
        'date_asc'  => ['id', 'asc'],
        'date_desc' => ['id', 'desc'],
        'amt_asc'   => ['total_order_cost', 'asc'],
        'amt_desc'  => ['total_order_cost', 'desc'],
    ];
    ...
    $company_listing_data = $results->map(function ($row) {
        ...
        return [
			...
			'acc_manager_name' => trim(admin_name($row->acc_manager)),
...
//credit approval in company 21-aug-2025
public function createPreApprovalCreditRequest(Request $request)
{
    $validated = $request->validate([
    ...
    // If currency column doesn’t exist, either add it or append to notes
    $notes = $validated['notes'] ?? '';
    $notes = trim($notes . " | Currency: " . $validated['currency']);

    $id = DB::table('pre_approval_credit_requests')->insertGetId([
        'comp_id'        => $validated['comp_id'],
        'requested_by'   => $validated['requested_by'],
        'payment_terms'  => $validated['paymentTerms'],
        'approval_amount'=> $validated['amount'],
        'notes'          => $notes,
    ...
    return response()->json([
        'status'  => true,
        'message' => 'Pre-approval credit request created successfully',
        'id'      => $id
    ]);
}
...
public function listPreApprovalCreditRequests(Request $request)
{
    $baseQuery = DB::table('pre_approval_credit_requests as p')
        ->leftJoin('tbl_comp as c', 'c.id', '=', 'p.comp_id')
        ->leftJoin('tbl_admin as a', 'a.admin_id', '=', 'p.requested_by')
		->leftJoin('tbl_currencies as tc', 'tc.currency_id', '=', 'p.currency')
        ->where('p.deleteflag', 'active')
->orderBy('p.id', 'desc');
    // Apply filters to base query (will be reused for list + counts)
if ($request->has('comp_id') && $request->comp_id != 'All' && $request->comp_id != '') {
        $baseQuery->where('p.comp_id', $request->comp_id);
    }

    if ($request->has('approval_status') && $request->approval_status != 'All' && $request->approval_status != '') {
        $baseQuery->where('p.approval_status', $request->approval_status);
    }

    if ($request->has('account_manager') && $request->account_manager != 'All'  && $request->account_manager != '') {
        $baseQuery->where('p.requested_by', $request->account_manager);
    }
    if ($request->has('financial_year')) {
        $fy = explode('-', $request->financial_year); // e.g. 2025-2026
        if (count($fy) == 2) {
            $fyStart = $fy[0] . '-04-01';
            $fyEnd   = $fy[1] . '-03-31';
            $baseQuery->whereBetween('p.request_date', [$fyStart, $fyEnd]);
        }
    }

    // 🔹 Clone base query for list
    ...
    $query->select(
        ...
        'p.request_date',
		'p.currency',
		'tc.currency_css_symbol',
        DB::raw("'Credit' as approval_for"),
        'c.comp_name as customer_name',
        DB::raw("CONCAT(a.admin_fname, ' ', a.admin_lname) as account_manager"),
        'p.approval_amount',
        'p.notes',
        'p.approval_status',
        'p.status',
    ...
    $requests = $query->orderBy('p.request_date', 'desc')
                      ->offset($offset)
                      ->limit($records)
                      ->get();

    // 🔹 Clone again for counts
    $countQuery = clone $baseQuery;
    $counts = $countQuery
        ->select(
            DB::raw("SUM(CASE WHEN p.approval_status = 'Pending' THEN 1 ELSE 0 END) as pending"),
            DB::raw("SUM(CASE WHEN p.approval_status = 'Approved' THEN 1 ELSE 0 END) as approved"),
            DB::raw("SUM(CASE WHEN p.approval_status = 'Rejected' THEN 1 ELSE 0 END) as rejected"),
            DB::raw("SUM(CASE WHEN p.approval_status IN ('Expired','Revoked') THEN 1 ELSE 0 END) as expired_revoked")
        )
        ->first();

    return response()->json([
        'status'        => true,
        'message'       => 'Pre-approval credit requests fetched successfully',
        'total_records' => $totalRecords,
        'current_page'  => (int) $pageno,
        'per_page'      => (int) $records,
        'requests'      => $requests,
        'counts'        => $counts
    ]);
}

public function updatePreApprovalCreditRequestStatus(Request $request)
{
    $validated = $request->validate([
        'id'      => 'required|integer',
        'status'  => 'required|in:PENDING,APPROVED,REJECTED,DOCS_REQUIRED,REF',
        'remarks' => 'nullable|string',
        'user_id' => 'required|integer'
    ]);

    $affected = DB::table('pre_approval_credit_requests')
...
public function invoice_delivery_method()
    {
        return response()->json(InvoiceDeliveryMethod::select('id', 'method_name')->get());
    }

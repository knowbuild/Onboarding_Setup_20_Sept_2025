...
public function vendorDetailsById(Request $request)
{
    ...
    $vendor = DB::table('vendor_master')->where('ID', $vendorId)->where('deleteflag', '!=', 'deleted')->first();
...
public function vendorOrderHistory(Request $request)
{
    ...
    $results = $orders->map(function ($order) {
        $invoice = DB::table('vendor_po_invoice_new')
            ->select('invoice_no', 'invoice_date', 'due_on', 'payment_paid_on_date','invoice_upload','awb_upload','boe_upload')

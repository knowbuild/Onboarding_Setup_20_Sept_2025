...
// Updated regular expression to capture image, PDF, and Excel files (XLSX)
preg_match('/data:(image\/(png|jpeg|jpg)|application\/(pdf|vnd.openxmlformats-officedocument.spreadsheetml.sheet));base64,/', $image, $matches);

// Default extension to 'pdf' if not found
if (isset($matches[2])) {
    $extension = $matches[2];  // For images (png, jpeg, jpg)
} elseif (isset($matches[3])) {
    if ($matches[3] === 'pdf') {
        $extension = 'pdf';  // For PDF files, no conversion
    } elseif ($matches[3] === 'vnd.openxmlformats-officedocument.spreadsheetml.sheet') {
        $extension = 'xlsx';  // For Excel files, save as XLSX
    } else {
        $extension = 'bin';  // Default binary extension for unrecognized types
    }
} else {
    $extension = 'bin';  // Default to bin if no match is found
}

// Remove the base64 prefix
$image = preg_replace('/^data:(image\/(png|jpeg|jpg)|application\/(pdf|vnd.openxmlformats-officedocument.spreadsheetml.sheet));base64,/', '', $image);
$image = str_replace(' ', '+', $image);
...
// Save the file to the server (as-is for PDF)
file_put_contents($externalPath . $imageName, $imageData);

// Prepare the path for the response
$dataArray["PO_path"] = $externalPath1 . $imageName; 
/*************************************************************************/	
...
$to_cc_email = "pawan@stanlay.com,accounts@stanlay.com";
...
if($acc_manager!='' && $acc_manager!='0' && $acc_manager!='All')
  { 
$acc_mg_search_ensure_month_sale="  and order_by='$acc_manager'";
  }
  else
  {
...
$total_confirmed_orders_counter			= total_orders_count_this_month_fy_yr($acc_manager,$q1_start_date_show,$q4_end_date_show,$currmonthsel,'5');//5,6,7
$total_closed_orders_counter			= total_orders_count_this_month_fy_yr($acc_manager,$q1_start_date_show,$q4_end_date_show,$currmonthsel,'6,7');//5,6,7
...
//echo "<pre>";
//print_r($result_sales_to_ensure);
/*if($result_sales_to_ensure["0"]->order_by!='' && $result_sales_to_ensure["0"]->order_by!='0')
{
$acc_manager_id								= $result_sales_to_ensure["0"]->order_by; //exit;
}
else
{
	$acc_manager_id							= ""; //exit;
...
$msg="true";
 return response()->json([            
		...
		'result_ensure_sale_month_count_month_wise' => $result_ensure_sale_month_count_month_wise,
			...
			"total_closed_orders_counter"=>$total_closed_orders_counter,				
...
public function company_directory_listing(Request $request)
{
    ...
    if ($admin_role_id == '0' || $admin_role_id == '5') {
        $acc_manager_filter = $acc_manager;
    } else {
        $acc_manager_filter = "0";
    }
    ...
    // Filter by status
    $search_filter_by_com_status = $com_status ? " AND key_customer = '$com_status' " : "";

    // View Favorite logic
    if ($view_fav == '1') {
        $comp_fav_ids = fav_comp_ids($currentuserid);
        $search_view_fav = $comp_fav_ids ? " AND id IN ($comp_fav_ids) " : "";
    } else {
        $search_view_fav = "";
    }

    // Filter by creation date
    $search_filter_by_comp_creation_date = "";
    if (!empty($comp_creation_date)) {
        switch ($comp_creation_date) {
            case '1': // Last 7 days
                $NewDate = date('Y-m-d 15:58:37.000000', strtotime('-7 days'));
                break;
            case '2': // Last 30 days
                $NewDate = date('Y-m-d 15:58:37.000000', strtotime('-30 days'));
                break;
            case '3': // Last 365 days
                $NewDate = date('Y-m-d 15:58:37.000000', strtotime('-365 days'));
                break;
            case '4': // Next 365 days (future records)
                $NewDate = date('Y-m-d 15:58:37.000000', strtotime('+365 days'));
                $search_filter_by_comp_creation_date = " AND create_date BETWEEN '$Today' AND '$NewDate' ";
                break;
            default:
                $NewDate = "";
        }
        ...
        if ($NewDate) {
            $search_filter_by_comp_creation_date = " AND create_date BETWEEN '$NewDate' AND '$Today' ";
        }
    }
    ...
    if (!empty($search_by_name)) {
        switch ($filter_by) {
            case '0': // Mobile
                $search = " AND (mobile_no LIKE '%$search_by_name%') ";
                break;
            case '1': // First or Last Name
                $search = " AND (fname LIKE '%$search_by_name%' OR lname LIKE '%$search_by_name%') ";
                break;
            case '2': // ID
                $search = " AND (id = '$search_by_name') ";
                break;
            case '3': // Multiple fields
                $search = " AND (comp_name LIKE '%$search_by_name%' OR fname LIKE '%$search_by_name%' OR mobile_no LIKE '%$search_by_name%' OR lname LIKE '%$search_by_name%') ";
                break;
            case '4': // Company Name
                $search = " AND (comp_name LIKE '%$search_by_name%') ";
                break;
        }
    }
...
$search_by_keyword= " AND comp_name like '%".$search_by."%' OR mobile_no like '%".$search_by."%' OR fname like '%".$search_by."%' OR id like '%".$search_by."%'   OR lname like '%".$search_by."%'  ";
	}
...
$search_by_keyword="";
    ...
    if ($acc_manager != ' ' && $acc_manager != '0' && $acc_manager > 0) {
        $search_filter_by_acc_manager = "AND acc_manager = '$acc_manager'";
    ...
    $total_records = DB::table('tbl_comp')
        ->whereRaw("deleteflag = 'active' $search $search_by_keyword $search_filter_by_acc_manager $search_filter_by_comp_creation_date $search_filter_by_com_status AND india_mart_co = 'no' $search_view_fav")
        ->count();

    // **Paginated Query with LIMIT**
    $sql_sub = "
        SELECT DISTINCT 
            id, salutation, office_type, parent_id, address, country, state, city, zip, cust_segment, fname, comp_name, lname, mobile_no, create_date, status, city, acc_manager, co_extn_id, co_city, co_division, key_customer
        FROM tbl_comp 
        WHERE deleteflag = 'active'  
        $search 
        $search_filter_by_acc_manager  
        $search_filter_by_comp_creation_date 
        $search_filter_by_com_status 
    ...
    $company_listing_data = $sql_sub_row->map(function ($row) {
        $acc_manager_id = $row->acc_manager;
        $admin_name = admin_name($acc_manager_id);     
        $customers_id = $row->id;
        ...
        return [
            ...
            'acc_manager' => trim($admin_name),
            'orders_count' => $comp_orders_count,
...
$searchRecord	= $search_filter_by_acc_manager.$search_filter_by_financial_year;
    ...
    // **Paginated Query with LIMIT**
      $sql_sub = "Select ID, received_date,financial_year,account_manager,save_send,approved_on,approved_by,tes_target,discount,actual_target,status from tbl_tes_manager where 1=1 $searchRecord  order by $order_by $order  LIMIT $from, $max_results"; //exit;
...
$sql_sub = "Select ID, status_update_reason,received_date,financial_year,account_manager,save_send,approved_on,approved_by,tes_target,discount,actual_target,status from tbl_tes_manager where ID = '$tes_id'"; //exit;
   ...
   $rowsub = DB::select(($sql_sub));
    ...
    $tes_discount	  = isset($rowsub->discount) ? $rowsub->discount : '';
	$status_update_reason	  = isset($rowsub->status_update_reason) ? $rowsub->status_update_reason : '';
	$tes_target	  = isset($rowsub->tes_target) ? $rowsub->tes_target : '';	
 ...
 $searchRecord = $search_filter_by_acc_manager . $search_filter_by_financial_year . $search_filter_by_tes_id;
   // **Query to Get Total Count (without LIMIT)**
    $total_records = DB::table('tbl_tes')
        ...
        ->count();
    // **Paginated Query with LIMIT**
    $sql_sub = "SELECT ID, tes_id, date_created, comp_name, comp_id, cosegmentid, cosegment, pro_id, pro_name, quantity, discount, price, sub_total, account_manager, cust_classification FROM tbl_tes WHERE 1=1 $searchRecord ORDER BY $order_by $order LIMIT $from, $max_results";
    $sql_sub_row = collect(DB::select($sql_sub));
    ...
    $grouped_data = $sql_sub_row->groupBy('comp_id')->map(function ($company_rows) {
        return $company_rows->groupBy('pro_id')->map(function ($product_rows) {
            return $product_rows->map(function ($row) {
                return [
...
DB::raw("CONCAT(tbl_admin.admin_fname, ' ', tbl_admin.admin_lname) AS admin_name"),
        ...
        ->leftJoin('tbl_admin', 'tbl_admin.admin_id', '=', 'tbl_order.order_by')  // Added join
...
public function pqv_listing(Request $request)
{
    ...
    if ($request->product_type_class_id) {
        $baseQuery->where('tbl_products.product_type_class_id', $request->product_type_class_id);
    }
 ...
 if($request->app_cat_id)
    ...
    // Clone the base query for counting records without pagination
    $countQuery = clone $baseQuery;

    // Get total count of records matching filters
    $num_rows_count = $countQuery->count('tbl_products.pro_id');
...
*/
 ...
 if($request->app_cat_id)
    ...
    // Get total count of records matching filters
    $num_rows_count = $countQuery->count('tbl_services.service_id');

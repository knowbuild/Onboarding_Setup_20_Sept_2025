...
public function vendorslisting(Request $request)
{
    ...
    // Current FY range
    $fyStart = now()->month >= 4 ? now()->startOfYear()->month(4)->startOfMonth() : now()->subYear()->startOfYear()->month(4)->startOfMonth();
    $fyEnd = $fyStart->copy()->addYear()->subDay();

    // Fetch vendor IDs now to match with PO data
    $vendorList = $query->get();

    // Get all PO data (no limit to FY)
    $allPOs = DB::table('vendor_po_final as vf')
        ->join('vendor_po_order as vo', 'vf.sup_ref', '=', 'vo.ID')
        ->where('vo.Confirm_Purchase', 'active')
        ->select('vo.VPI as vendor_id', 'vf.Date', 'vf.Prodcut_Qty', 'vf.Prodcut_Price')
        ->get();

    $poSummary = [];

    foreach ($allPOs->groupBy('vendor_id') as $vendorId => $records) {
        $fyTotal = $records->filter(fn($r) => $r->Date >= $fyStart && $r->Date <= $fyEnd)
                           ->sum(fn($r) => $r->Prodcut_Qty * $r->Prodcut_Price);

        $last = $records->sortByDesc('Date')->first();

        $poSummary[$vendorId] = [
            'purchase_current_fy' => round($fyTotal, 2),
            'last_purchase_date'  => $last?->Date,
            'last_purchase_amount' => $last ? ($last->Prodcut_Qty * $last->Prodcut_Price) : 0,
        ];
    }

    // Sort vendorList by PO Date if requested
    if ($sortField === 'last_purchase_date') {
        $vendorList = $vendorList->sortBy(function ($vendor) use ($poSummary) {
            return $poSummary[$vendor->ID]['last_purchase_date'] ?? '0000-00-00';
        }, SORT_REGULAR, $sortOrder === 'desc');
    } else {
        $vendorList = $vendorList->sortBy($sortField, SORT_REGULAR, $sortOrder === 'desc');
    }

    // Manual pagination
    $paginated = $vendorList->forPage($page, $perPage)->values();
    ...
    // Final data mapping
    $vendors = $paginated->map(function ($vendor) use ($countries, $paymentTerms, $priceBases, $productCounts, $poSummary) {
        ...
        $contactsFromDB = DB::table('vendor_contacts')
            ...
            ->map(function ($group) {
                return $group->unique(function ($item) {
                    return strtolower(trim($item->email)) . '|' . trim($item->mobile);
                })->values();
            });
...
public function getPurchaseOrders(Request $request)
{
    ...
    $poNoSearch = $request->input('po_no');
    ...
    if (!empty($poNoSearch)) {
        $query->where('vendor_po_final.PO_ID', $poNoSearch);
    }

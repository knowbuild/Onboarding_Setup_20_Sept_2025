...
{  
    ...
    function pending_account_receivables_by_aging_not_yet_due($acc_manager = 0, $company_name = '', $fyStartDate = null, $fyEndDate = null)
    {
        ...
        // Account manager filter (integer only)
        if (!empty($acc_manager) && is_numeric($acc_manager) && $acc_manager != '0') {
            $where .= " AND tti.prepared_by = " . (int)$acc_manager . " ";
        }
        ...
        if (!empty($fyStartDate) && !empty($fyEndDate)) {
            $where .= " AND tti.invoice_generated_date BETWEEN '$fyStartDate' AND '$fyEndDate' ";
        }
    ...
    public function receivablePieSeries(Request $request)
    {
        try {
            ...
            $limit           = $request->query('limit', 10); // Default to 10 records
            ...
            // Account Manager Filter (as integer ID)
            if (!empty($acc_manager) && is_numeric($acc_manager) && $acc_manager != '0' && $acc_manager != 'All') {
                $where .= " AND tti.prepared_by = " . (int)$acc_manager . " ";
            }
            ...
            if (!empty($invoice_number)) {
                $where .= " AND tti.invoice_id = " . (int)$invoice_number . " ";
            }
            ...
            $sortColumn = $sortable[$sortKey] ?? 'balance_due';

            // SQL query with dynamic limit
            $sql = "SELECT 
                        tti.invoice_id,
                        tti.cus_com_name,
                        tti.prepared_by,
                        ...
                        IFNULL(SUM(tpr.payment_received_value), 0) AS payment_received,
                        SUM((tti.freight_amount + tti.sub_total_amount_without_gst + tti.total_gst_amount) * tti.exchange_rate) - 
                            IFNULL(SUM(tpr.payment_received_value), 0) AS balance_due
                    FROM tbl_tax_invoice tti
                    INNER JOIN tbl_supply_order_payment_terms_master s ON tti.payment_terms = s.supply_order_payment_terms_id
                    LEFT JOIN tbl_payment_received tpr ON tti.invoice_id = tpr.invoice_id AND tpr.deleteflag = 'active'
                    LEFT JOIN tbl_tax_credit_note_invoice ttcni ON ttcni.invoice_id = tti.invoice_id
                    WHERE $where
                    GROUP BY tti.cus_com_name  
                    ORDER BY $sortColumn $sortValue";
            
            // Add limit only if not 'all'
            if (strtolower($limit) !== 'all' && is_numeric($limit)) {
                $sql .= " LIMIT " . (int)$limit;
            }

            $result = \DB::select($sql);
            if ($request->boolean('count', false)) {
                $countSql = "SELECT COUNT(DISTINCT tti.cus_com_name) as company_count FROM tbl_tax_invoice tti 
                    INNER JOIN tbl_supply_order_payment_terms_master s ON tti.payment_terms = s.supply_order_payment_terms_id
                    LEFT JOIN tbl_tax_credit_note_invoice ttcni ON ttcni.invoice_id = tti.invoice_id
                    WHERE $where";
                $countResult = \DB::select($countSql);
                $companyCount = isset($countResult[0]->company_count) ? (int)$countResult[0]->company_count : 0;
                return response()->json([
                    'status' => 'success',
                    'count' => $companyCount
                ]);
            ...
            return response()->json([
                ...
                'meta' => [
                    ...
                    'record_count' => count($datapie),
                    'limit_applied' => strtolower($limit) === 'all' ? 'No limit' : $limit,
                    'filters_applied' => array_filter([
                        'acc_manager' => $acc_manager ?: null,
                        'financial_year' => $financialYear ?: null,
                        'company_name' => $company_name ?: null,
                        'invoice_number' => $invoice_number ?: null,
                        'team_members' => $team_members ?: null,
                        'aging_min' => $aging_min ?: null,
                        'aging_max' => $aging_max ?: null
                    ])
                ]
            ], 200);
        } catch (\Exception $e) {
            return response()->json([
                'status' => 'failed',
                'message' => 'Failed to fetch receivable pie series: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    public function topReceivables(Request $request)
    {
        ...
        $searchAccManager   = $request->input('acc_manager', 0);
        $searchAging        = $request->input('aging', '');
        $companyName        = $request->input('company_name', '');
        $invoiceNumber      = $request->input('invoice_number', 0);
        ...
        // For count, do NOT order by alias
        $totalRecords = $query->count();

        // Now apply orderBy for data fetch
        $result = $query->orderBy($sortColumn, $sortValue)
    ...
    public function accountsReceivables(Request $request)
    {
        ...
        $hvcSearch      = $request->input('hvc_search', ''); // '1' for HVC only
        ...
        $receivables = $rows->map(function ($row) {
            ...
            return [
                ...
                'company_name'         => $row->cus_com_name,
                'account_manager'      => $this->getAdminName($row->prepared_by),
                'total_invoice_amount' => html_entity_decode($row->currency_html_code) . ' ' . moneyFormatIndia($totalInvoiceAmount, 2, '.', ''),
                'part_payment_received'=> html_entity_decode($row->currency_html_code) . ' ' . moneyFormatIndia($partPaymentReceived, 2, '.', ''),
                'balance_amount'       => html_entity_decode($row->currency_html_code) . ' ' . moneyFormatIndia($totalInvoiceAmount - $partPaymentReceived, 2, '.', ''),
    ...
    public function ShortTermReceivables(Request $request)
    {
            ...
            $financialYear  = $request->input('financial_year', 0);
            ...
            // Filters
            // Financial Year filter (assumes format '2024-2025')
            if (!empty($financialYear)) {
                [$fyStart, $fyEnd] = explode('-', $financialYear);
                $fyStartDate = $fyStart . '-04-01';
                $fyEndDate = ((int)$fyEnd) . '-03-31';
                $query->whereBetween('tti.invoice_generated_date', [$fyStartDate, $fyEndDate]);
            }
            if (!empty($accManager)) {
                $query->where('tti.prepared_by', $accManager);
            }
    ...
    public function overdueReceivables(Request $request)
    {
        ...
        // If count requested, return only count
        if ($request->boolean('count', false)) {
            $total = $query->count();
            return response()->json([
                'status' => 'success',
...
{
    // Extract filters as per UI (account manager and financial year)
    $acc_manager    = $request->input('acc_manager', null);
    $financial_year = $request->input('financial_year', null);

    $rupee = html_entity_decode("&#8377;") . "\u{00A0}"; // Rupee symbol with non-breaking space

    // Helper: Parse financial year to date range
    $fyStartDate = $fyEndDate = null;
    if (!empty($financial_year)) {
        [$fyStart, $fyEnd] = explode('-', $financial_year);
        $fyStartDate = $fyStart . '-04-01';
        $fyEndDate = ((int)$fyEnd) . '-03-31';
    }

    // Pass filters to all relevant methods (filters come from UI dropdowns)
    $not_yet_due = $rupee . moneyFormatIndia(
        $this->pending_account_receivables_by_aging_not_yet_due($acc_manager ?? 0, '', $fyStartDate, $fyEndDate)
    );

    $pending_account_receivables = $this->pending_account_receivables(
        $acc_manager ?? 0,
        $fyStartDate,
        $fyEndDate,
        0,
    ...
    $payment_received_by_aging = $this->payment_received_by_aging(
        $acc_manager ?? 0,
        0,
        0,
        ''
    ...
    $pending_account_receivables_one = $this->pending_account_receivables(
        $acc_manager ?? 0,
        $fyStartDate,
        $fyEndDate,
        0,
    ...
    $payment_received_by_aging_one = $this->payment_received_by_aging(
        $acc_manager ?? 0,
        1,
        30,
        ''
    ...
    $pending_account_receivables_two = $this->pending_account_receivables(
        $acc_manager ?? 0,
        $fyStartDate,
        $fyEndDate,
        0,
    ...
    $payment_received_by_aging_two = $this->payment_received_by_aging(
        $acc_manager ?? 0,
        31,
        60,
        ''
    ...
    $pending_account_receivables_three = $this->pending_account_receivables(
        $acc_manager ?? 0,
        $fyStartDate,
        $fyEndDate,
        0,
    ...
    $payment_received_by_aging_three = $this->payment_received_by_aging(
        $acc_manager ?? 0,
        61,
        90,
        ''
    ...
    $pending_account_receivables_three_plus = $this->pending_account_receivables(
        $acc_manager ?? 0,
        $fyStartDate,
        $fyEndDate,
        0,
    ...
    $payment_received_by_aging_three_plus = $this->payment_received_by_aging(
        $acc_manager ?? 0,
        91,
        365,
        ''
    ...
    // Calculate actual disputed receivables from tbl_invoice_disputes with filters
    $disputedAmount = $this->getDisputedReceivablesTotal($acc_manager ?? 0, $fyStartDate, $fyEndDate);
...
// ...existing code...
    ...
    public function Collections(Request $request)
    {
        ...
        $query = PaymentReceived::active();

        // Filter by account manager (prepared_by on invoice)
        if (!empty($accManager) && is_numeric($accManager)) {
            $invoiceIds = DB::table('tbl_tax_invoice')
                ->where('prepared_by', (int)$accManager)
                ->where('deleteflag', 'active')
                ->pluck('invoice_id')
                ->toArray();
            if (!empty($invoiceIds)) {
                ...
                // No invoices for this manager, return zeroed response
                return response()->json([
                    'status' => 'success',
                    'ytd' => html_entity_decode("&#8377;") . '0',
                    'mtd' => html_entity_decode("&#8377;") . '0',
                    'dso' => '0',
                ]);
            }
        }

        // Filter by financial year (format: 2024-2025)
        if (!empty($financialYear)) {
            [$fyStart, $fyEnd] = explode('-', $financialYear);
            $fyStartDate = $fyStart . '-04-01';
            $fyEndDate = ((int)$fyEnd) . '-03-31';
            $query->whereBetween('payment_received_date', [$fyStartDate, $fyEndDate]);
        }

        $currentMonth = date('m');
        $currentYear = date('Y');

        // YTD (Year to Date) for selected filters
        $ytd = (clone $query)->whereYear('payment_received_date', $currentYear)
            ->sum('payment_received_value');

        // MTD (Month to Date) for selected filters
        $mtd = (clone $query)->whereMonth('payment_received_date', $currentMonth)
            ->sum('payment_received_value');

        // DSO placeholder (implement actual logic as needed)
        $dso = 0;
    ...
    /**
     * Get total disputed receivables amount from tbl_invoice_disputes with optional filters
     * @param int|null $acc_manager
     * @param string|null $fyStartDate
     * @param string|null $fyEndDate
     * @return float
     */
    private function getDisputedReceivablesTotal($acc_manager = null, $fyStartDate = null, $fyEndDate = null)
    {
        ...
        if (!empty($acc_manager) && is_numeric($acc_manager)) {
            $query->where('tti.prepared_by', (int)$acc_manager);
        }
        if (!empty($fyStartDate) && !empty($fyEndDate)) {
            $query->whereBetween('tti.invoice_generated_date', [$fyStartDate, $fyEndDate]);
        }

        return $query->sum('tid.disputed_amount'); // Use actual disputed amount, not full invoice amount
    }

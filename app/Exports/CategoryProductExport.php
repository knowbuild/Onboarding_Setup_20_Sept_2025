...
class CategoryProductExport implements FromArray, WithHeadings, WithEvents
{
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $warrantyOptions = empty($warrantyList) ? '"No warranty available"' : '"' . implode(',', $warrantyList) . '"';
                      ...
                      $gst_vatList = GstSaleTypeMaster::active()->pluck('gst_sale_type_tax_per')->toArray();
                $gst_vatOptions = empty($gst_vatList) ? '"No GST/VAT available"' : '"' . implode(',', $gst_vatList) . '"';
    ...
    // Apply dropdown to gst_vat column (E2:E101)
                for ($row = 2; $row <= $totalRows; $row++) {
                    $cell = 'E' . $row;
                    $validation = $sheet->getCell($cell)->getDataValidation();
                    $validation->setType(DataValidation::TYPE_LIST);
                    ...
                    $validation->setAllowBlank(false);
                    $validation->setShowDropDown(true);
                    $validation->setFormula1($gst_vatOptions);
                 }
